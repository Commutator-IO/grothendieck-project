...
%
% The revision of 2026-09-22 (the Revised line above). The first pass read
% the photocopy alone and misread
...
$SC \to SC'$ : en vertu du critère cohomologique de Artin-Mazur, il f\supplied{au}t
et s\supplied{uffit} que pour tout système de coéfficients $F'$ sur $C'$,
...
A $C$ on peut asso\supplied{ci}er le topos $\widetilde{C}$, qui varie de façon
...
inversant le\supplied{s} foncteurs qui sont des homotopismes. Ces isomorphismes
son\supplied{t} fonctoriels en $X$. Il en résulte formellement qu'un morphisme
...
conditi\supplied{on} de le supposer $\neq 0$ dans le cas de la réciproque) ; la
partie dir\supplied{ecte} vaut aussi avec un anneau de coefficients pas néc.\ 
constant, mais consta\supplied{nt} tordu. Je pense que ce résultat (facile) doit
pouvoir se généraliser ai\supplied{nsi}.

Soit $f : X \to X'$ un morphisme de topos qui soit tel que pour tout faisceau
loc.\ const.\ sur $X'$, $f$ induise un isomorphisme sur les cohomolog\supplied{ies}
(avec cas non commutatif inclus). Supposons que \struck{\ill{}} \add{$X$ et
$X'$} soit \emph{locale\supplied{ment} homotopiquement trivial}, i.e. que pour tout
...
$H^{j}(U, G) \to H^{j}(U_{i}, G)$ son\supplied{t} nuls pour $1 \leq j \leq n$. Alors
...
homomor\supplied{p}hisme de groupes $G \to H$ (ici de\supplied{s} groupes $\pi_{1}$ de
...
En fait, $X$ et $X'$ étant loc.\ hom\supplied{.}\ triviaux, les conditions suivantes
...
(do\supplied{nc} on ne prend ici que des coéfficients commutatifs).
\end{itemize}

\marginal{?}
J'ignore si on peut dans a) se borner aux systèmes locaux commutatif\supplied{s}.
...
\page{6}
éventuellement muni de la sous-catégorie pleine de tous les systèmes loc\supplied{aux}
...
condition d'ex\supplied{a}ctitude familière (transformer sommes amalgamées en
produit\supplied{s} fibrés). De même la catégorie $(\mathrm{Tricat})$ des
...
$C_{1} = \underline{\mathrm{Fl}}(C)$, avec structure\supplied{s} $p_{1}, p_{2}$,
évidentes (d'où une $(\mathrm{Cat})$-cat\supplied{ég}orie dont les composantes
semi-simpliciales supérieures sont les catégories
$\underline{\mathrm{Fl}}^{n}(C)$ des $n$-flèches de $C$ …) L'ensemble
bi-semisimplicial correspondant a pour composantes les ensembl\supplied{es}
$\mathrm{Hom}(\Delta^{n} \times \Delta^{m}, C)$. Par exemple, les objets de
$X_{11}$ sont les carrés commutatif\supplied{s} de flèches de $C$, qu'on peut
interpréter (de deux manières différentes) co\supplied{mme}

\page{7}
morphismes de flêches.

Notons une symmétrie évidente dans la catégorie $(\mathrm{Bicat})$,
interprété\supplied{e} en termes d'ensemble\supplied{s} bi-semisimpliciaux, obtenue en
échangeant les lign\supplied{es} et les colonnes dans de tels ensembles bi…
Un ensemble bisemisimpl\supplied{ici}al qui … peut donc de deux façons
différentes être considéré comme déf\supplied{i}nissant une bicatégorie. De même le
...
correspondant exactement au\supplied{x} axiomes des $(\mathrm{Cat})$-catégories. De
cette façon, la catégorie des 2-catégor\supplied{ies} est équivalente à la
catégorie des $(\mathrm{Cat})$-catégories qui ont une « catégo\supplied{rie}
...
$n$-catégorie en termes d'ensembles $n$-semi-simplici\supplied{aux}.
...
d'en déte\supplied{r}miner tous les composants à l'aide de $C_{0}$ et $C_{1}$ .
...
tous les groupe\supplied{s} abéliens de $(\mathrm{Cat})$ ainsi. Un complément
important est que toute catégori\supplied{e} de Picard \struck{(resp.\ et
commutative)} est équivalente (au sens technique qu'on ne précis\supplied{e} pas
...
On notera qu'on a une classification beaucoup plus restrein\supplied{te} que pour les
$H$-espaces (resp.\ les $H$-espaces homotopiquement comm.) mod\supplied{ulo}
...
l'asso\supplied{cia}tivité (resp.\ et de la commutativité), puisqu'on ne demande pas
seulemen\supplied{t} l'existence d'isomorphismes d'associativité (qui suffirait
pour avoir u\supplied{n} groupe dans la catégorie homotopique
$(\mathrm{Cat})' \simeq (\mathrm{Ssimpl})'$, modulo l'existence d'unités et
d'invers\supplied{es}) mais qu'on suppose \emph{donnés} ces isomorphismes, de façon
...
\page{9}
On doit trouver une approximation meilleure d'un cran à la classifi\supplied{ca}tion
topologique en prenant des $(2\text{-Cat})$-groupes resp.\ des
$(2\text{-Cat})$-group\supplied{es} abéliens, ou plus généralement des
« Gr-2-catégories » resp.\ « Grab-2-catégories » . (NB Une Grab-catégorie
définit une Grab-2catégor\supplied{ie} comme une catégorie définit une
2-catégorie ; du point de vue semi-simpli\supplied{ci}al, cette opération serait
...
« tuage des groupes d'homotopie en dim $> 1$ »). Pour l\supplied{a} notion
groissière (Gr ou Grab) l'idée de ma lettre à Verdier était qu\supplied{'on} doit
trouver la même chose que les tronqués au cran 2, dans la catégorie
\emph{homotopi\supplied{que}} des $H$-espaces --- i.e. ce qu'on déduit des
$H$-espaces par tuage des group\supplied{es} d'homotopie en dimensions $> 2$ .
...
(Demander à Quillen relations exactes entre objets-groupes et objet\supplied{s}
connexes de la catégorie homotopique ponctuée : forment-ils des catégories
équival\supplied{en}tes par le foncteur espace des lacets ?).
...
version bi-semisimpli\supplied{ci}ale du tapis de Dold, on trouve que la catégorie
de ces objets est équivalente à la catégorie des bicomplexes de chaines de
groupes abéli\supplied{ens} qui sont nuls en dehors du carré $0 \leq i, j \leq 1$.
La \struck{colonn\supplied{e}} \add{\uncertain{cond.}} que la première ligne corresponde
...
catégorie de $(2\text{-Cat})$-groupes abéliens est équivalente à celle de\supplied{s}
...
préciser) entre deux telles 2-Grab-caté\supplied{go}ries strictes, en termes d'un
complexe $R\mathrm{Hom}$.

L'idée qui se dégage de cette approche, et que Quillen on espère \add{va}
expliciter, c'est que la notion de groupe \emph{abélie\supplied{n}} dans
...
naturell\supplied{e} de $n$-équivalence d'un côté, et dans la catégorie dérivée de
l'autre. Cec\supplied{i} doit pouvoir se préciser en utilisant des
$\mathrm{Hom}^{\bullet}$ resp.\ des $R\mathrm{Hom}^{\bullet}$.

Si maintenant $C$ est une catégorie avec un « produit tensoriel »
assoc\supplied{ia}tif (et éventuellement unitaire), i.e. un bifoncteur avec des
...
seraient par déf\supplied{ini}tion les $K_{n}(C)$. Un cas intéressant est celui où
$C$ est une catégorie ave\supplied{c} produits finis (par exemple une catégorie
additive) et $\otimes$ est le produit, \supplied{[}Mais on notera que dans le cas d'une
catégorie non seulement additi\supplied{ve} mais abélienne, il y a une notion plus
adéquate du $K_{0}$ , qui devrait corr\supplied{es}pondre à une notion plus
...
\page{11}
qu'il conviendra de poser les définitions adéquates). --- Peut-être Quill\supplied{en}
...
correspo\supplied{n}dre à $C$. Ce n'est certainement pas en général l'ensemble
...
« homoto\supplied{pe} » au foncteur nul, puisqu'il suffit d'avoir un homomorphisme
...
Gr-catégor\supplied{ie} i.e. admet des inverses, alors ce soit bien la construction
« naïve » $SC$ qu\supplied{i} est la bonne, puisqu'on trouve bien un $\pi_{0}$ et un
...
(pas nécessairem\supplied{ent} orientables), les morphismes étant les classes
...
f'_{\bullet}\, g'^{\bullet}$ (on pose $f_{\bullet} = F_{\bullet}(f)$,
$f^{\bullet} = F^{\bullet}(f)$) . La question est de trouver une $B$ et un
couple $(F_{\bullet}, F^{\bullet})$ qui soient « universels » dans u\supplied{n}
...
formule est tapée « $g^{\bullet} f_{\bullet} = f!\, g'^{\bullet}$ » : le « ! »
est l'apostrophe et le point frappés l'un sur l'autre, soit $f'_{\bullet}$ ; de
même plus bas pour $F'_{\bullet}$. Même lecture à l'exemplaire annoté (p.~43)
et au dossier 111.}
...
class\supplied{es} d'éléments de $C$ au dessus de $X$, pour la relation de
...
somme de $B(X)$ est la somme disjointe (c'est une loi de grou\supplied{pe} où tout
...
évidente (tou\supplied{te} variété sur $X$ est sur $Y$), et
$f^{\bullet} : B(Y) \to B(X)$ par produit fibré (util\supplied{i}sant encore le
th.\ de transversalité). De cette façon, $B(X)$ devient u\supplied{n}
foncteur-anneau \add{(gradué)} contravariant en $X$, et un foncteur
groupe covariant e\supplied{n}

\page{13}
$X$. De plus, on a la formule \add{(du type
$g^{\bullet} f_{\bullet} = f'_{\bullet}\, g'^{\bullet}$ ci-dessus, impliquant la
...
don\supplied{né} par les graphes sur les flêches. Composé avec la symmétrie
précédente on trouve un foncteur $F^{\bullet} : C \to \underline{B}$. On
vérifie enfin que le couple $(F_{\bullet}, F^{\bullet})$ satisfait aux
conditions envisagées dans l'alinéa 1 (ceci aus\supplied{si} devrait être formalisé
...
$(F'_{\bullet}, F'^{\bullet})$ à valeurs dans une catégorie $B'$, il faut trouver une
...
$q : Z \to Y$, on lui assoc\supplied{ie} un élément du deuxième membre de $(*)$
comme le composé
\[
...
seulement $T$ et la décomposition $\mathrm{bord}(T) = Z_{\uncertain{0}} \amalg Z_{1}$) qu'il
\note{l'indice $0$ de $Z_{0}$, à l'encre sur l'exemplaire annoté (p.~44), n'est
qu'une trace sur cette photocopie}
...
Cela nous ramène à prouv\supplied{er} que le composé
...
Sur la str\supplied{u}cture des $B_{\bullet}(X)$. Ce sont tous des algèbres graduées
sur $B_{\bullet}(\mathrm{pt}) = \Lambda$ , qui est une algèbre graduée à
degrés positifs, réduit\supplied{e} à $\underline{\mathbb{F}}_{2}$ en degré 0. C'est
...
$1 \in H^{0}(Z)$ par l'homomorphisme de Gys\supplied{in} associé à $Z \to X$. Cet
homomorphisme par Thom est surjectif (toute cla\supplied{s}se de cohomologie
...
tous les modules gradués libre\supplied{s} de type fini à générateurs en degrés
$\leq 0$ seulement. C'est u\supplied{ne}
...
copie.}
Pour trouver ces derniers, on peu\supplied{t} si on veut paraphraser la
...
lui-même est pleinement fidèl\supplied{e} ce qui permet alors, par une construction
formelle standard, de com\supplied{plé}ter $\widetilde{\underline{B}}$ en une
...
de \struck{type} \add{présentation} f\supplied{inie} sur $\Lambda$. (\add{qui est
...
plus haut, mais en prena\supplied{nt} l'équivalence homologique (via le graphe) des
morphismes de variété\supplied{s} au lieu de l'équivalence homotopique.
...
trou\supplied{ve} tous les vectoriels gradués de dim.\ finie sur
$\underline{\mathbb{F}}_{2}$ . C'est un peu tro\supplied{p} trivial comme structure,
...
e\supplied{n} termes d'opérations de Steenrod, cf plus bas). On prouve de même (par
...
add\supplied{i}tive dont les objets sont ceux de $C$, les flêches sont les
...
est fort simple\supplied{.} Nous allons en donner une interp\supplied{r}étation
...
anticommutative i.e. commutative (car.\ 2 !), associative et unitaire. So\supplied{n}
...
d'aille\supplied{urs} le foncteur des $\otimes$-automorphismes additifs du foncteur
...
est-el\supplied{le} déjà nécessaire ?) Une autre façon de le voir est de dire que le
...
de dimensi\supplied{on} finie sur $\underline{\mathbb{F}}_{2}$ . (Quelle est la
structure de $G$, qui est prolisse ; d'après Quillen, il serait
pro-unipotent …). Ici $G$ bien sûr joue le rôle d'u\supplied{n} groupe de
...
nécessa\supplied{i}rement propres (mais $f_{\bullet}$ n'étant supposé défini que
