...
par \struck{\ill{}} \uncertain{variétés} loc.\ noeth.\ avec top.\
Zariski).}
...
$H^{0}$ et $H^{1}$ (y inclus dans le cas non commutatif\ldots). On prouve la
...
$(\mathrm{Ssimpl})$ des objets qui \ldots{} , et de même pour les catégories
...
$\underline{\mathrm{Fl}}^{n}(C)$ des $n$-flèches de $C$ \ldots) L'ensemble
...
échangeant les lign\add{es} et les colonnes dans de tels ensembles bi\ldots{}
Un ensemble bisemisimpl\add{ici}al qui \ldots{} peut donc de deux façons
...
celles-ci \ldots{} Prenant la catégorie somme $C$ des
...
elle ne se \uncertain{réalise} \ill{} que par un $(\mathrm{Cat})$-g.\
\uncertain{non} \uncertain{commut.}}
...
isomorphismes d'associativité (mais sans inverse \ldots), on peut lui associer
...
au cas des correspondances générales \ldots). On vérifie enfin qu'il a la
...
construction de la catégorie des motifs, en considérant l'objet
$\Lambda(-1)$ de $\underline{B}$ conoyau de $p^{*} : e \to S$, où
$p : S^{1} \to e$ est l'application du cercle sur un point. C'est un objet de
poids 1 (le poids est défini par le degré des générateurs, changés de signe).
On constate aussitôt que le foncteur
$M \mapsto M(-1) = M \otimes \Lambda(-1)$ de $\underline{B}$ dans lui-même est
pleinement fidèl\add{e} ce qui permet alors, par une construction formelle
standard, de com\add{plé}ter $\underline{B}$ en une catégorie
$\widetilde{\underline{B}}$ où $\Lambda(-1)$ devient inversible. Il se trouve
alors que dans cette dernière les $\underline{\mathrm{Hom}}(X, Y)$ internes
existent et sont isomorphes à $\check{X} \otimes Y$, \ldots{} en fait , on
trouve maintenant \add{la} catégorie de \emph{tous} les modules gradués libres
de type fini sur $\Lambda$. Bien sûr, cette catégorie n'est pas encore
abélienne pour autant, puisque $\Lambda$ n'est pas un corps ; si on veut
définir la catégorie abélienne engendrée, on s'attend à trouver la catégorie
des modules de \struck{type} \uncertain{présentation} f\add{inie} sur
$\Lambda$. (\uncertain{qui est cohérent bien que $\Lambda$ ne soit pas
noethérien, car $\Lambda$ est lim d'anneaux noeth.\ à homs de transition}
\emph{plats}) \struck{est-il noethérien, i.e. le nombre de générateurs est-il
fini ?}).
\note{« quasi-abélienne », « $\Lambda(-1)$ » et « si on veut » sont insérés à
la main, de même que le tilde de $\widetilde{\underline{B}}$, avec un signe
en marge ; la question tapée sur la
noethérianité de $\Lambda$ est biffée et remplacée par la réponse écrite à la
main au-dessus et à la suite, signalée d'un signe en marge.}
...
d'égards. Pour trouver un équivalent plus étroit, il conviendrait de
...
précédente, sont barrées de grandes croix ; « co » y est
ajouté au-dessus de « homologiques ».}

Pour trouver une catégorie universelle, on associe à toute $X$ l'ensemble
$S(X)$ des sous-variétés de $X$, modulo équivalence homologique. Donc
...
les $S(X)$ ne sont pas stables par Gysin\ldots). D'où une catégorie
add\add{i}tive dont les objets sont ceux de $C$, les flêches sont les
\uncertain{él.\ des} $S(X \times Y)$. Il semble qu'elle doive jouer le rôle
...
dual gradué est une algèbre graduée anticommutative donc commutative, avec application diagonale associative et unitaire ; il
est réduit à $\underline{\mathbb{F}}_{2}$ en degré zéro. Donc il correspond à
un schéma en groupes affine $G$ sur $\underline{\mathbb{F}}_{2}$ , qui est
d'aille\add{urs} le foncteur des $\otimes$-automorphismes additifs du foncteur
$H^{\bullet}$ sur $C$ \uncertain{qui opèrent trivialement sur} \ill{} $H^{1}$
\ill{}.
\note{« stables » remplace à la machine un mot biffé ; la définition tapée qui
suit est barrée à la main d'un trait sinueux, avec un signe en marge ; « i.e.\
commutative (car.\ 2 !) », « graduée anticommutative donc », placé au-dessus de « commutative », et « additifs »
sont insérés à la main, et la phrase est achevée à la main au bas de la page,
où l'on ne lit sûrement que « $H^{1}$ » suivi d'une parenthèse et d'un mot
biffé.}
...
pro-unipotent \ldots). Ici $G$ bien sûr joue le rôle d'u\add{n} groupe de
...
\note{« fixe » est écrit à la main au-dessus d'un mot tapé puis effacé.}
\marginal{\ill{}}
\note{un bloc de huit ou neuf lignes courtes, écrites en oblique dans la marge
de gauche et reliées par une flèche aux mots « groupe de Galois motivique »,
reste illisible sur la photographie.}

Quillen donne une généralisation de la construction d'une catégorie
...
\uncertain{stable} sur les \emph{morphismes} envisagés , via un morphisme
...
$Y$ \ldots{} . Les $B^{H}_{m}(X)$ ($X$ compact \uncertain{disons}) peuvent se
décrire homotopiquement à la Thom, comme les groupes d'homotopie
\uncertain{stables} d'espaces $\underline{\mathrm{Hom}}(X, MH(N - m))$ ----- .
\note{la dernière phrase, depuis « Les $B^{H}_{m}(X)$ », est de sa main, à la
suite du texte tapé.}
