% Lecture modernisée — fonds Alexandre Grothendieck, cote 161-1, lot 1
% (feuillets 1-19).
%
...
\begin{resume}
Une chemise étiquetée « Catégories » au crayon, dix-neuf feuillets sans
...
exactement. Les feuillets 3 à 5 isolent ces deux noyaux, montrent que
...
équivalence entre elles. Les feuillets 6 et 7 poursuivent avec une question
...
« Or c'est vrai sauf erreur… », conclut le feuillet — et c'est vrai.

Le deuxième chantier (feuillets 9 à 12) attaque une gêne familière :
...
Le troisième chantier (feuillet 13, page 2 d'une note dont la première
...
tour une catégorie. Le feuillet est barré d'un grand trait oblique — le
geste de qui a reporté la note ailleurs.

Le dernier chantier (feuillets 15, 17, 19) est le plus proche des topos :
...
les limites ? Le feuillet 15 réfute d'abord une fausse piste par un
...
\section*{La partie fixe d'une adjonction (feuillets 3 à 5)}
...
Le feuillet 5 dresse la liste des conditions équivalentes suivantes — on
...
de cette condition est barrée sur le feuillet, avec en marge « à revoir
pour simplifier » ; nous la retenons parce qu'elle est correcte et que le
feuillet suivant s'en sert.}
\end{enumerate}
...
Deux exemples dégénérés ouvrent le feuillet 6 : si $E$ est ponctuelle, $v$
...
\section*{Localisations et objets locaux (feuillets 6 et 7)}
...
essentielle de $\alpha$ ? Le feuillet vérifie que
\[
...
$\Phi(\Sigma)$ — notation sans rapport avec le $\Phi(A)$ des feuillets
suivants. Au passage, le feuillet 7 écrit deux fois
$\mathrm{Hom}_{E_1}(Y, \alpha Z_1)$ là où seul $\mathrm{Hom}_E$ a un sens ;
lapsus de plume, corrigé ici.}

\section*{Le produit tensoriel sans parenthèses (feuillets 9 et 10)}
...
cohérence datent de 1963--1965 (Mac Lane, Bénabou) ; les feuillets ne sont
...
\textbf{Propriété universelle} (la Proposition du feuillet 9) : pour toute
...
biaisée. Noter aussi le NB du feuillet 10 : $\Phi$ est une monade —
$\pi$ appliqué dans $\Phi(C)$ lui-même aplatit une famille de familles en
la famille réunion disjointe.}

\section*{Contractions, inverses et le signe (feuillets 11 et 12)}

\subsection*{Familles à couples contractés}

Le feuillet 11 enrichit $\Phi(C)$ : soit $\Phi^{c}(C)$ la catégorie des
...
monoïdales à duaux — évaluations et « ficelles » comprises, le feuillet 12
en dessine.
...
identifications de $\pi((L_i))$ avec $L^{\otimes n}$ ; le feuillet écrit
...
$\pi(L, L') \simeq \mathbf{1}$ sur le produit non ordonné. Le feuillet 12
...
\]
« Relations entre $\varphi_{L',L}$ et $\psi_{L',L}$ ? » demande le
feuillet, qui s'interrompt sur le carré de naturalité de la symétrie et
...
\section*{Théories et leurs constructions (feuillet 13)}

Un feuillet isolé — la page 2 d'une note dont la première page n'est pas
...
finies — théories essentiellement algébriques, esquisses de Ehresmann. Le
feuillet manipule ces objets comme allant de soi, en des termes qui lui
...
structure--sémantique.\footnote{Le membre de droite du premier diagramme du
feuillet, rogné par la marge, n'a pas pu être lu en entier ; la
transcription porte le détail.}

\section*{Prolongements : préfaisceaux, densité, topos (feuillets 15, 17, 19)}

\subsection*{Une fausse piste réfutée (feuillet 15)}
...
cela, note le feuillet, que $\varphi$ soit un objet \emph{injectif} : que
...
gauche. La phrase du feuillet (« on sait que ») indique un fait déjà acquis
dans son entourage ; les feuillets ne sont pas datés.} Réciproque ? »
...
Giraud.\footnote{Aux hypothèses de taille près, que le feuillet ne discute
...
barrée sur le feuillet — c'est bien la réciproque qu'il visait.}

\subsection*{Densité : le calcul complet (feuillets 17 et 19)}

Les deux derniers feuillets font le calcul général qui sous-tend tout cela.
...
\textbf{Lemme.} \emph{Si $\varphi$ vérifie {\upshape(*)}, alors pour tout
...
isomorphisme.}\footnote{Le feuillet écrit $\beta\varphi(Y)$ — lapsus pour
...
cofinalité, en général, reste ouverte sur ces feuillets.
