...
%
% Revised 2026-09-23 (Opus 5.5), checked on the facsimile (400 dpi):
%   p. 16  added \note{sic, pour « schéma »} to « soit un faisceau » in the
%          second Cor (the word stays as written).
%   p. 20  the letters of the left-hand square and of « De plus, si Y' -> Y »
%          are the gothic 𝔛, 𝔜 of p. 21 (batch 2's \mathfrak), not \mathcal;
%          the note said they stood for the spectra — it is the roman X, Y of
%          the right-hand square that are the spectra. Letters and note fixed.
...
\textbf{Cor} (Supposant $S = \operatorname{Spec} k$) Pour qu'un foncteur $\mathrm{Alg}_k \to (\mathrm{Ens})$ soit un faisceau\note{sic, pour « schéma » : la phrase exige la notion définie, non la condition a) ; le mot est bien « faisceau » sur la page, suivi d'un signe (lettre ou croix) qu'on ne lit pas} il f. et il s. que \struck{\ill{}} \add{\uncertain{ce soit un}} faisceau, et qu'on le puisse recouvrir par des sous-faisceaux ouverts affines …
...
\]
est un plongement, à le foncteur image \struck{inverse associé est} directe associé est \struck{$F \longmapsto$} \struck{$(X \longmapsto a(FX))$} $u \longmapsto j_* \circ u$ qui est bien pl. fidèle.
...
\end{array}
\]
\note{à gauche, des lettres gothiques --- le même tracé qu'à la p. 21, où $\mathfrak{X} \to \mathfrak{Y}$ est un monomorphisme d'espaces algébriques affines --- ; à droite, des romaines, $X$, $Y$, qui sont les spectres dont la phrase dit que le carré est cartésien. La flèche du haut et celle de droite du carré gothique sont en pointillé, celle du bas porte deux pointes}

De plus, si $\mathfrak{Y}' \to \mathfrak{Y}$ est une imm. fermée (resp. ouverte) --- plus généralement si celle correspond à $A \to A_S^{-1}$\note{lu $A S^{-1}$, localisation}, disons l'homéomorphisme dans, \struck{le diagramme} des $=$ pour $\mathfrak{X}' \to \mathfrak{X}$, donc la top. de $X'$ est la top. induite.
