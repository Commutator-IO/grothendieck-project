...
%    tassé for the conditions batch 1 reads « préféré » (p. 19) and « bornés »
%    (p. 20), on the authority of p. 21, which names them so.
...
%          <f, eps_s * gcheck>.
%  p. 20   tassé => prétassé, not the converse (« C'est impliqué par » is an
%          uncertain reading); counterexample ours.
...
\begin{enumerate}
  ...
  pas.\footnote{La page 52 porte « 82 » à la main. Le thème --- des couples
...
combinaison des $F_k$.\footnote{La transcription lit $E_i = [i, i]$ et
$E_1 = \varepsilon$ ; le sens demande $[-i, i]$ et $E_0 = \{0\}$. La lettre
des coefficients de la troisième relation est lue $\gamma$ sans certitude.}
...
\end{itemize}
On dit alors que le couple $(f, g)$ est \emph{prétassé}.\footnote{La
transcription lit ici « couple \emph{préféré} », le titre étant
« Couples préférés », au premier mot surchargé. La page 21 appelle
« prétassé » exactement la condition dont le corollaire ci-dessous donne le
cas $(f, f)$, et l'en-tête du lot 2 de la transcription nomme la section de
la page 19 « Couples prétassés ». On suit la page 21 ; la lecture des pages
19 et 20 serait à revoir sur le fac-similé.} La relation n'est pas
...
\end{itemize}
On dira que $(f, g)$ est \emph{tassé}.\footnote{Le titre de la section est lu
« Couples \emph{bornés} », d'une lecture incertaine ; la page 21 dit
« tassé » de la condition a) pour les parties finies, et l'en-tête du lot 2
de la transcription nomme cette section « Couples tassés ». Le second membre
...
Un couple tassé est prétassé : la première inégalité de $f' * g' \prec f * g$
est $\|f' * g'\|_\infty \leqslant \|f * g\|_\infty$. La réciproque est fausse,
et c'est pourquoi l'on ne suit pas ici la lecture « C'est impliqué par
$(f, g)$ préféré ».\footnote{La lecture de « C'est impliqué par » est
incertaine ; le sens exact est « cela implique ». Contre-exemple (nôtre) :
