...
% 2 f' < f, 3 couples préférés, 4 couples bornés, « condition K ») on
...
\section*{3.\ \emph{Couples préférés}}

\note{le premier mot du titre est surchargé ; lu « Couples », que confirme la définition plus bas}
...
\emph{Déf} On dit alors que le \struck{\ill{}} couple $(f,g)$ est \emph{préféré}

([Il n'est pas clair que la relation soit symétrique \struck{telle signifie $\forall s$} mais OK si $G$ commutatif])

\emph{Cor} $f\in\mathbb{R}[G]^+$, \struck{symétrique} $(f,f)$ est préféré (\struck{\ill{}} \struck{$f$ et $\check f$ \ill{}} $\exists\,s\in G$) tel que $\check f=\varepsilon_s*f$ ($\Longleftrightarrow\check f=f*\varepsilon_s$)
...
\page{20}
5.\ Donc si $f$ et $g$ sont sym.\ à translation près, la condition $(f,g)$ préféré est une condition \emph{symétrique} en $f,g$
\marginal{Cette condition est inv.\ par translations. \struck{\ill{}} $(f,g)$ est bien $\check g,\check f$ aussi. Donc si $f,g$ sont sym.\ modulo translation, alors $(f,g)$ bonne \uncertain{ssi} $(g,f)$ bonne}
\note{le chiffre « 5 » en tête surcharge un autre ; la note de droite est d'une écriture serrée, lue en partie}

\section*{4.\ \emph{Couples \uncertain{bornés}}}

\note{le second mot du titre est d'une lecture incertaine}
...
\uncertain{C'est impliqué par} $(f,g)$ est \emph{préféré}.

\emph{Cor} Soit $S\subset\mathbb{R}[G]^+$, $S$ \emph{stable par} $*$. \struck{Pour que} Conditions équivalentes :

a) \struck{\ill{}} \uncertain{Tout} couple $(f,g)\in S\times S$ est borné, \struck{il suffit}
