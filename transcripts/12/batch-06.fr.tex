...
% dual. His « eff^t » is eff\add{t} (effectivement), « str^t » str\add{t}
...
\item[d)] Si $F$ est \struck{spécial} eff\add{t} admissible \emph{constant
tordu} à poids $\subset P$, et si $i \in P \Rightarrow i \leqslant n$, alors
$\check{F}(\struck{n})$ est à\add{ eff\add{t}} admissible (constant tordu) à
...
soit eff\add{t} adm.\ à poids $\subset P$, il faut et il suffit que l'on
...
4.1 Un faisceau $F$ est dit eff\add{t} admissible \emph{de niveaux}
...
--- On dit que $F$ est eff\add{t} admissible « de poids $\subset P$, et de
...
« eff\add{t} adm.\ de poids $\in P$ » $\Rightarrow$ « id.\ et à niveau
$\leqslant \nu$ ».
...
\begin{enumerate}
\item[a)] Stabilité de « eff\add{t} adm.\ de poids $\subset P$ et niveau
$\leqslant \nu$ » par sous-faisceaux constr., quotients constructibles,
sommes finies, extensions.
\item[b)] $F_{i}$ « eff\add{t} adm.\ de poids $\subset P_{i}$, niveau
$\leqslant \nu_{i}$ » pour $i = 1, 2$ implique $F_{1} \otimes F_{2}$
« eff\add{t} adm.\ de poids $\subset P_{1} + P_{2}$, niveau
$\leqslant \nu_{1} + \nu_{2}$ ».
\item[c)] Stabilité par chgt de base.
\item[d)] Si $F$ \add{eff\add{t}} admissible \emph{constant tordu}, « à poids
\struck{$k$} \add{$\nu + 2k$} et niveau $\leqslant \nu$ » (où $k \geqslant 0$,
$\nu \geqslant 0$), \struck{et si} alors pour tout $n \geqslant \nu + k$
\add{(soit $n = \nu + k + h$, $h \geqslant 0$)}, $\check{F}(n)$ est « eff\add{t}
...
…, $R^{i}f_{!}(F)$ est eff\add{t} admissible \struck{\ill{}} à poids
...
\add{même si} on suppose seulement que $F$ est eff\add{t} admissible de poids
$j$. Par suite, si on suppose $F$ eff\add{t} admissible de poids
$\leqslant j$, alors on a un énoncé : $R^{i}_{!}f(F)$ est eff\add{t}
...
\marginal{alors par semi-simplicité : $F$ eff\add{t} $=$ str.\ spécial}
...
$\mathbb{Z}$, et \struck{\ill{}} si $F$ est eff\add{t} admissible, alors
$R^{i}_{!}f(F)$ est eff\add{t} admissible.
...
$F$ est \struck{\ill{}} str\add{t} spécial de poids $j$ (resp.\ eff\add{t}
adm.\ de poids $j$, si l'on admet conjecture de $\tfrac{1}{2}$simplicité),
alors $R^{i}f_{*}(F)$ est \struck{\ill{}} eff\add{t} admissible de poids entre
...
\struck{str\add{t} spécial} \add{effectivement admissible} sur $X$, de poids
...
$E_{2}^{p0} = R^{p}h_{*}(\mathbb{Q}_{\ell,Y'})$ est eff\add{t} admissible de
...
Si $F$ \uncertain{str\add{t}} spécial de poids $j$ :
\[
...
\emph{chaînes}), en utilisant la théorie de Dold\add{-Puppe}. La difficulté
