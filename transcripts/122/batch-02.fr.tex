...
\textbf{NB}. On a un homom\supplied{orphisme} $(u_0, u_1, u_2)$ des triangles (e) dans les
...
\]
et qui induit un isom\supplied{orphisme} $u_0$ (grâce aux \ill{} d'identification
...
$K^\bullet$ parfait rel\supplied{ativement à} $\Lambda$}
...
\note{l'exposant de $K^\bullet$, lu « ens », est douteux ; dans le triangle (c) de la page 19 (lot 1), le même exposant a été lu comme un petit « $\omega$ » (ou « w »), et les deux lectures sont à confronter aux pages. Sous le premier
...
\textbf{NB}. En prenant des coefficients généraux, loc\supplied{alement}
...
hyp\supplied{othèse} de \struck{régularité} non-singularité sur $W$, les suites
...
Supposons que $W$ soit \emph{une sphère} (de dim\supplied{ension} $2n+1$). Alors
...
\marginal{NB $W$ sph\supplied{ère} coh\supplied{omologique} $\Longleftrightarrow$
...
\ill{} : les conditions de finitude naturelles (pour $A$ loc\supplied{alement}
noeth\supplied{érien}), sur les systèmes inductifs $a$-adiques, ou ess\supplied{entiellement}
