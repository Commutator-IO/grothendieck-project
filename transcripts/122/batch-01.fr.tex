...
$P$ variété à bord \supplied{para}compacte, purement de dimension $n$
...
Ainsi, on a des hom\supplied{omorphismes}
\[
...
\ill{} un hom\supplied{omorphisme} de suites exactes
\[
  \begin{array}{ccccccc}
...
$P \cap Q = \partial P = \partial Q$ est purement de dim\supplied{ension} $n - 1$.
...
La suite exacte de coh\supplied{omologie} relative pour $W$, $R$, $W - R$ donne
\[
...
Soit $W$ une variété \struck{topologique}, diff\supplied{érentiable}.
...
de bases $S$, $T$ des variétés diff\supplied{érentiables} compactes $S$, $T$, de
...
L'hyp\supplied{othèse} \ill{} \uncertain{pour} $\partial P$ \uncertain{est} un fibré
