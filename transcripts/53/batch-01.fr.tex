...
ouvert (p.\,ex.\ …). Alors $X$ est réunion \ill{} de parties
...
$G^0 \times W \to W \times W$} …\ $x \in W \Leftrightarrow y \in W$.
...
3)\quad \emph{Groupes \struck{\ill{}} radiciels}\qquad Structure des \uncertain{types}
...
\add{interligne : tore $\mathbf{G}_m^r$ tordu …\ tordus (?)}
...
\emph{Cas abéliens}\qquad VA, $\mathbf{G}_a$, $\mathbf{G}_m$,
...
$e \times W$, il contient $G^0 \times W$…\ (Ici, on n'utilise pas
...
est réduit, $u$ est plat, donc générisant (donc générisant …). Si on ne
...
2)\quad \emph{Groupes connexes}, \emph{Séparables}
...
régulier …\ c)$\Rightarrow$b) car si $A \to B$ hom.\ local \uncertain{fid.}\
plat d'anneaux loc.\ noeth., \ill{} $B$ régulier $\Rightarrow A$ régulier…\
...
\textbf{NB}\quad $X$ \struck{\ill{}} géom.\ régulier$/k$ en un \emph{point} $x \in X$ :
...
\emph{maximal} $x \in X$, $\underline{O}_{X,x}$ est réduit et $k(x)$ est
une extension \emph{séparable} de $k$ [$\Leftrightarrow$ extension étale
...
\textbf{NB}\quad On a une série analogue pour la \emph{normalité géométrique},
...
alors c'est vrai pour tt schéma normal \add{(lisse)} sur $k$…\ On a
...
\uncertain{rétrocompact} (tant que $G$ \emph{séparé}), \struck{\ill{}}
...
\emph{Rappel sur la construction de $\pi_0(X/k)$} lorsque
...
\emph{composantes} \emph{connexes} de $X$, pour toute telle
...
\textbf{Déf}\quad On dit alors que $A$ est \emph{séparable} sur $k$ (ou
géom.\ \emph{réduit})
...
C'est un critère « géométrique ». \emph{Stabilité par produit}

\marginal{(2) \struck{\ill{}} Irréductibilité}
...
$\Longleftrightarrow X_{k'}/k'$ géom.\ irréd. / Stabilité par produit

\marginal{(3) \struck{\ill{}}}
...
\textbf{Déf.}\quad On dit alors que $X$ est \emph{géom.\ connexe} sur $k$.
...
$G_{\mathrm{réd}}$ pour $k$ \emph{parfait} : un sous-groupe fermé de $G$
...
\emph{Application}\quad Soit $X$ un schéma sur $k$, supposons d'abord
...
\emph{Généralisation}\quad On suppose $X$ quasi-compact, ou somme de
...
\emph{Dualité de Cartier}\quad $\underline{\mathrm{Hom}}(G, \mathbf{G}_m)$
...
\emph{Th.}\quad Tout groupe comm.\ fini loc.\ libre \add{$G$} est réflexif
pour \add{$\mathbf{G}_m$}. I.e.\ Son dual $G^{*}$ est fini loc.\ libre comm.,
d'où une \emph{involution} $G \mapsto G^{*}$ de la catégorie des
groupes comm.\ loc.\ libres. Elle correspond à la dualité des bialgèbres
bicommutatives sur $S$.

\emph{Ex}\quad $\mu_n \leftrightarrow \mathbb{Z}/n\mathbb{Z}$,\quad
$\alpha_p \leftrightarrow \alpha_p$ \quad | \quad Mais aussi
$D(M) \longleftrightarrow M_S$

Marche $\pm$ pour les schémas en \emph{groupes} \struck{affines} \ill{}
...
tordues…\ Si on veut inclure le groupe $\mathbf{G}_a$, il faut…
