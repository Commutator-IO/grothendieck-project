...
Y_{\overline{k'}}$ définissant un \supplied{épi}morphisme $H^{n-3}(Z,
...
aux fibres\supplied{.} On fera attention que même si $X$ est lisse et géom.\ connexe,
...
\add{ce sont} \uncertain{les} puissance\supplied{s} $m$-ème\supplied{s} \uncertain{des}
...
$p$-linéaire\supplied{s} opérant sur les $H^i(X, \underline{\mathcal{O}}_X)$. Quitte
...
réduit.}} Soit $r$ la \add{somme des} degré\supplied{s} des composantes irréductibles
...
conclusions de 7.2\supplied{,} sous la forme affaiblie qu'on peut satisfaire à la
...
\supplied{endo}morphisme $p$-linéaire $f$, $t$ un point de $S$ et $s$ une
