...
\uncertain{les} extens\supplied{ions} \uncertain{de} $K$ sont abél\supplied{iennes}
...
Soit $\overline{K}$ clôture a\supplied{lgébrique} de $K$.
...
si $k$ car \uncertain{$\neq 0$}, $V$ pas néces\supplied{sairement} complet,
...
« in\supplied{jectif} », qui est biffé d'un trait}
...
\textbf{Proposition} Pour que $F$ soit exact à gauche, il f\supplied{aut} et
s\supplied{uffit} que $X$ soit un $\pi$-torseur à droite.
...
\textbf{Corollaire} Pour que $F$ soit équivalence, il f\supplied{au}t et
s\supplied{uffit} que $\pi' \xrightarrow{\sim} \mathrm{Aut}_{\pi}(X)$, i.e. que $X$
