...
\]
du type $(3,3)$, et un \struck{isom.} \add{syst. trans. d'}isom. entre les
...
Considérons alors le \add{$\Gamma_\omega$-}torseur :
\[
...
\]
qui se factorise par un \add{iso}morphisme
\[
  (5) \qquad
...
\begin{itemize}
  ...
  \item[b)] la famille des \add{$\Lambda^{*}$-torseurs} $(T_x)_{x \in P}$ ---
    i.e. le \add{$\Lambda^{*}_P$-}torseur relatif
  ...
  \item[d)] la donnée de la relation de disjonction dans $P = P(V)^!$
    ...
    \add{bi-}stable par l'opération de $\Lambda^{*}$ (i.e. $u, v$ disjoints
...
\add{est} ess.\ surjectif\add{, \uncertain{\textbf{Foncteur réalisation topologique}}} : dim. \ill{}
