...
un système fond. de voisinages de $a$ dans $X/A$. Les $V_t^{0} = t V^{0}$, où $V^{0} = V \smallsetminus B$, forment un syst. fond. de voisinages\supplied{ ;} les images inverses $W_t$ de $V_t$ dans $X$ forment un syst. fond. de voisinages de $A$ dans $X$. Pour tout tel $W_t$, \struck{\ill{}} $\mathcal{S}_t = \dot{W}_t$ \marginal{cl$\smallsetminus$i}\note{petit croquis au-dessus de la ligne ; la glose « cl $\smallsetminus$ i », reliée à $\dot W_t$ par un trait, est d'une lecture incertaine} est un fermé de $X$, et \struck{\ill{}} posant $W'_t = X - V_t$, on a
\[
