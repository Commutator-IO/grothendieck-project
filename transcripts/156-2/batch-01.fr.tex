...
% kept. X̄ for the cut space and Ã for its preimage of A are his. His
...
% a) on page 17. Two variances are on the page and not repaired: page 10's
% text asserts that the faille structure on a segment is unique while its own
% margin lists the family A = [0,α], B = [β,1]; page 12's sketch labels a
% lieu D^1 where B^1 is expected. The lemma of page 6 does not state
% Z = Z' ∪ Z''.
%
...
\page{4}
\marginal{\textbf{NB} Pour une faille réduite $(X', \{a,b\})$, \struck{\ill{}} $X'$ est connexe, ainsi que $X' \smallsetminus \{b\}$ et $X' \smallsetminus \{a\}$ (connexes dans $X'$), mais jamais $X' \smallsetminus \{a,b\}$ (ex. $S^1$ et deux pts)}
...
\note{la note marginale contredit l'unicité affirmée dans le texte ; la contradiction est sur la page}
