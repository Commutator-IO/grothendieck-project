...
\ill{} « de Pic » — les deux ou trois mots qui
...
« th.\ de Hopf $+$ \uncertain{nullité} de $H^{i}(A_{0},
\mathcal{O}_{A_{0}})$ p\supplied{ou}r $i > \dim A_{0}$ » — soit
...
\note{interlinéaire au-dessus de la ligne : « (soient $F$, $G$
quasi-coh.\ sur $X$, $Y$ ; $G \to F$ un $\varphi$-morphisme) »}
...
\note{à droite du cadre : « $\pi : A \times_{S} A \to A$ \ill{}
la loi de composition \ill{} i.e.\ $\sigma$ ». Le noyau est donc
...
« id », et sous $\mathrm{pr}_{2} i_{1}$
« $\varepsilon_{A/S}$ » ; les trois accolades
