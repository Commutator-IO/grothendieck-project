...
% pass; its header was read at the end of this pass (see « trièdre » below).
%
...
% distingué ») was first read « tridron » by the batch-2 pass; re-cropped at
% 900 dpi on pp. 23, 25 and 26, it is « tri » + a dot for the accent +
% « dres », the shape of « icosaèdre » on p. 26, and both batches now read
% « trièdre ». It stays \uncertain at its first occurrence here. G_s^+ here is G_s in batch 2's summary.
%
...
\struck{incidentes} de $s$, non incidentes …
...
Pour un \uncertain{polyèdre} combinatoire, …
...
majorés …\ Pour tt sommet \ill{} au moins trois arêtes et trois
faces.
...
4 b) et de 4 a), pour face \uncertain{cela résulte de} …
...
2. Énumération. NB des sommets $s = 12$ ; NB des arêtes $a = 30$,
$2a = ps = qf$ ; NB des faces $f = 20$, $p = 5$, $q = 3$
\note{trois lignes alignées sous « NB » sur la page}
...
\quad B) — « polyèdres combinatoires »
...
\quad Transitivité des groupes des automorphismes sur sommets, …\ et
...
sommets nommés $u_1$, $u'$, $v'_1$, $w'_1$ (?), $v_1$, $w_1$, $u'_1$, …,
...
\]
antipodisme $a(s,u) = (\varepsilon s, \varepsilon u)$ …\
...
$u', v'$, $s, t$, $\Pi_{s,u}$, $\Pi_{t,v}$, … ; « ou » (C$_2$), la même
...
droite « géométrie d'incidence et ens.\ à opérateurs … »}
...
$E = \operatorname{Rep}$ dans $S \times A$ et $A \times F$ …
...
aussi de polyèdre combinatoire régulier …)
...
de $\Pi_0$, est défini à isom.\ unique près …
...
\uncertain{plus précis} …\ polygone) ; les $\bar\sigma$ engendrent $G$.
...
\operatorname{Aut}(\Pi)$, $\Phi$ = ens.\ des facettes de $\Pi$ …
\begin{enumerate}
...
rotation …) Alors \struck{\ill{}} $T$ est un torseur sous $G_s^+$ ---
