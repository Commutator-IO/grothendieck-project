...
\noindent 1) On peut \ill{}
\[
  U_Z = q^{-1}(V') \longleftarrow V' \cap Z' .
\]
\note{la flèche est tracée vers la gauche sur le feuillet}

\noindent a) $X_n \cap U_Z = q_n^{-1}(V')$ \uncertain{est schématiquement dense dans $\hat{X}$}

\noindent b) $X'$ est l'\uncertain{adhérence schématique} \ill{} de la famille des
$q_n(X_n) \subset X'$, \ill{}

\noindent c) La famille des $V'_n$ \add{(}\uncertain{est schématiquement dense dans $X'$}\add{)}
\marginal{i.e. en un $\hat{V'}$}

\noindent 2) Cas \uncertain{où} $V' \supset Z$, i.e. $f$ est étale aux points de $Z$
\add{(}et il existe une partie ouverte $U' \subset V'$ \uncertain{contenant} $Z$,
...
\noindent $c_1$) $V'$ schématiquement dense dans $X'$,

\noindent $c_2$) $\hat{V'}$ schématiquement dense dans $U'$, i.e.\ill{}
...
\noindent a) \emph{Surjectif} : il suffit de prouver que
...
\noindent b) \textbf{Mono.} Soient \struck{\ill{}} $k$ $\mathbb{F}_p$-alg.,
...
\noindent $g_b = u\,g_a$ donc $f_b = u f_a$, $u \in \operatorname{Aut}(\mathbb{G}_{a,k})$.
...
\noindent c) \emph{$\varphi_n$ \struck{est} propre.} \ill{} \uncertain{il faut prouver} \ill{}
\struck{\ill{}}
...
\xrightarrow{\ \sim\ } \operatorname{Hom}(J\alpha_p, J\,\text{faisceau})$}
...
\[
  ...
  = \prod_{(\alpha_1, \dots, \alpha_{n-1}) \in \mathbb{F}_p^{n-1}}\Bigl(X_n + \sum_1^{n-1}\alpha_i X_i\Bigr)
    \prod_{(\alpha_1, \dots, \alpha_{n-2}) \in \mathbb{F}_p^{n-2}}\Bigl(X_{n-1} + \sum_1^{n-2}\alpha_i X_i\Bigr)\cdots
\]
\[
...
\]
\struck{En effet} \add{[}Il est immédiat que $Q_n^0 \simeq$ « sous-groupes finis
...
\noindent 1) \emph{Surjection.} Soit $a = (a_0, \dots, a_{n-1}) \in k^n$.
...
\noindent 2) \emph{À fibres finies.} En fait, on voit tout de suite que les fibres
...
$\mathbb{F}_p[A_1, \dots, A_{n-1}, \Delta_n] \subset \mathbb{F}_p[X_1, \dots, X_n]^{S\Gamma_n}$)
...
\]
\add{[}ceci a un sens, grâce à : $\bigl(\overset{n}{\Lambda}\check{E}\bigr)^{p-1} \simeq \mathcal{O}_S$,
