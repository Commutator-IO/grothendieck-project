...
%   pp. 1-5     spherical maps with a marking <-> rational functions P/Q
%               normalised at 0, 1, oo, unramified off {0,1,oo}, ramification
%               <= 2 over 1 (Belyi maps; dessins); coefficients algebraic,
%               Aut(C) acts through Gal(Qbar/Q). Pp. 3, 5 cancelled scratch.
...
%               of segments and cores, anglets, the quasi-segment restart.
...
%          classifies hypermaps (bipartite maps). The page's question « P'=0
...
Le dossier s'ouvre sur une remarque de quelques lignes, devenue célèbre sous
une autre forme. Tracez sur une sphère un graphe dont les arêtes ne se
croisent pas : c'est une \emph{carte}. On peut alors faire de la sphère une
surface de Riemann et construire une fraction rationnelle qui envoie les
sommets sur $0$, les milieux d'arêtes sur $1$, les faces sur l'infini, et
n'a pas d'autre valeur critique que ces trois-là ; une fois trois points
marqués, elle est unique, et réciproquement une telle fraction redessine la
carte. Grothendieck en tire que les coefficients de ces
...
topologiques. La page s'arrête au milieu d'une phrase. C'est le point de
départ de ce que l'\emph{Esquisse d'un programme}, en 1984, appellera les
\emph{dessins d'enfants}.
...
l'autre. Grothendieck écrit cette théorie trois fois. La première rédaction
travaille avec les coupures elles-mêmes. La deuxième et la troisième
remplacent chaque coupure par ce qu'elle produit, deux « hémisphères » fermés
...
croisent pas. La dernière rédaction code tout cela par un ensemble fini
...
Ces pages montrent une manière de faire. Les définitions s'y corrigent en
écrivant : « si elles sont distinctes » s'ajoute en interligne quand la
démonstration en a besoin, « strictement » vient se placer devant « non
parallèles » dans la marge d'une page avant d'être défini dix pages plus
loin. Ce qui n'est pas encore compris est étiqueté comme tel ---
« Structure à étudier », « Intuition », « il faut vérifier que c'est
transitif » --- plutôt que glissé sous une formule. Et quand une rédaction ne
tient plus, il en commence une autre, numérotée.
...
\begin{enumerate}
  ...
  sur la sphère munies d'un repère correspondent aux fractions rationnelles
  normalisées en $0$, $1$, $\infty$, non ramifiées hors de ces trois valeurs
  et ramifiées au plus à l'ordre $2$ au-dessus de $1$ (page 2) ; leurs
  ...
  \item \emph{Pages 25 à 29}, un nouveau départ : les « dispositions
  ...
  emboîtées de segments et leurs cœurs, « anglets », et une reprise pour un
  « quasi-segment ».
...
ouverts, les \emph{faces} ; on la considère à isotopie près. Un \emph{repère}
...
marquer un sommet (au-dessus de $0$), un milieu d'arête (au-dessus de $1$) et
une face (au-dessus de $\infty$) : c'est notre lecture. La page ne dit pas
s'ils doivent être incidents ; la normalisation ne le demande pas.}. La page
énonce trois descriptions équivalentes de ces objets.

\textbf{Premier dictionnaire.} Les cartes sphériques orientées repérées
correspondent aux fractions rationnelles $f \colon \mathbb{P}^1_{\mathbb{C}}
\to \mathbb{P}^1_{\mathbb{C}}$ qu'il appelle \emph{spéciales} :
\[
...
au-dessus de $1$ un milieu d'arête. On a intégré la condition à la
définition.}. Les sommets sont les points au-dessus de $0$, avec pour degré
l'indice de ramification ; les faces, les points au-dessus de $\infty$ ; les
arêtes, les points au-dessus de $1$ d'indice $2$, l'arête étant la
composante de l'image réciproque du segment $[0,1]$ qui passe par ce point\footnote{Un point
au-dessus de $1$ d'indice $1$ est l'extrémité libre d'une demi-arête : c'est
ce que la page admet en demandant l'indice \emph{au plus} $2$. Quand on
exige l'indice exactement $2$, on parle aujourd'hui de fonction de Belyi
« propre ». Le dictionnaire est de nous ; la page n'énonce que
l'équivalence.}.

C'est une forme du \emph{théorème d'existence de Riemann}. Une carte fait de
la sphère privée des sommets, des milieux d'arêtes et des centres de faces un
revêtement fini de la sphère privée de trois points ; ce revêtement porte une
...
d'enfant}\footnote{Les deux noms sont postérieurs à la page : « dessin
...
« strict.\ » devant « non parallèles » : c'est la notion que la page 36
définira\footnote{La note est d'une lecture très incertaine ; on n'en garde
...
C'est la construction inverse de celle de la Version I, qui partait des
...
scissions définies par les arêtes d'un arbre. Rien n'indique que
Grothendieck connaissait ce travail, publié dans un recueil d'archéologie
quantitative. Le rapprochement, et l'affirmation que les deux constructions
sont inverses, sont de nous.}.

\pagerange{31}{39}
\section*{V. Décompositions $n$-aires : la Version II (pages 31 à 39)}

La troisième rédaction, titrée et numérotée, reprend tout avec des
...
C'est la condition qui, dans la Version I, venait de la connexité.}.
...
paire d'hémisphères n'est disjointe. Dans la Version I, où $\Sigma$ désignait
les composantes ouvertes, l'équivalence de la page 14 était juste.}. La marge
...
l'emboîtement. Tout cela est postérieur ou contemporain ; rien n'indique que
Grothendieck l'ait connu. L'identification des quartettes ordonnés aux
...
page ne compte ni n'ordonne. Rien n'indique que Grothendieck ait connu ce
travail. Le vocabulaire « non croisé » est invariant par renversement du
...
\pagerange{51}{52}
\subsection*{La reprise sur un quasi-segment (pages 51 et 52)}

Les pages 51 et 52, à l'encre noire, reprennent les notions des pages 41 à 47
