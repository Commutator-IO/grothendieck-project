...
%   leaf's label script-Q_M on the left agrees with page 9's convention, its
%   label script-P_M on the right does not, and the reading uses page 9's.
...
% — page 11: the question's « Tor_i(L, F) (L in P^k) » pairs two contravariant
%   functors; the reading writes Tor_i(F, L') with L' in (P°)^k.
% — page 12: « M |-> M (x)_k a » read with x; Proposition b) is stated with
%   small sums exact in N (AB4), which its proof uses and the page does not
%   state there.
%
% Revised 2026-09-23 (Opus 5.5), after the find-novelty pass. Changed:
% — header, page 5: only the label script-P_M departs from page 9's
%   convention, not both; the page-5 footnote says so;
% — page 2 footnote: « Le listing est daté du 16 septembre 1982 » removed;
%   the stamp, re-checked on the facsimile, is on the listings of pages 4
%   and 12, not on that leaf, and dates nothing but a lower bound;
% — page 11: the question's Tor now has the right variance, Tor_i(F, L') with
%   L' in (P°)^k, and a footnote gives what the page has;
% — page 12: Proposition b) now assumes small sums exact in N, and its proof
%   says where; footnote on the supplied hypothesis and on the page's
%   illegible hypothesis b). The earlier « relativement projectives » of the
%   transcription was a misreading of « résolutions projectives », so the
%   reading's « par résolutions projectives de F ou de L' » stands.
%
...
flèches obliques $\eta_{0}, \ldots, \eta_{n-1}$. Ce feuillet ne porte pas de
date ; le tampon « 16 SEP 82 » est celui des listages des pages 4 et 12.}
...
$Q^{k}_{M}$ : la première étiquette s'accorde avec cette convention, la
seconde non ; on suit la page 9.}, pour $M$ complète et cocomplète,
\[
...
$\mathrm{Tor}_{i}(F, L')$, $L' \in (P^{\circ})^{k}$, pour $i > 0$ ?\footnote{La
page écrit « $\mathrm{Tor}_{i}(L, F)$ ($L \in P^{k}$) ». Un $L$ de $P^{k}$ est,
comme $F$, contravariant sur $P$, et ne s'apparie pas avec lui ; le produit
$*_{k}$ que ces Tor dérivent apparie $F$ avec un objet de $(P^{\circ})^{k}$,
comme le $L'$ de la formule 1 et celui de la page 12. On corrige la
variance ; « $i > 0$ » est ajouté.}

\pagerange{12}{12}
\section*{Le produit dérivé (page 12)}

La première moitié de la question a une réponse positive, et la seconde une
réponse positive sous une hypothèse de platitude, pourvu que les sommes soient
exactes ; la page le dit, dans ses deux marges, à cette dernière hypothèse
près.
...
\item[b)] Si de plus les petites sommes sont exactes dans $N$ (une somme de
monomorphismes y est un monomorphisme), et si, pour tout projectif $x$ de $N$,
le foncteur $Ab_{k} \to N$, $V \mapsto V \otimes_{k} x$, est exact --- $x$
est « $k$-plat » ---, alors pour tout projectif $\Phi$ de $P^{k}_{N}$ le foncteur
...
objets, et une somme de suites exactes est exacte par hypothèse. Du côté de
$L'$, $F \mapsto F *_{k} L''$ est exact pour $L''$ projectif : pour $L''$
représentable par $b$, c'est $F \mapsto F(b)$, et l'on passe aux sommes par la
même hypothèse ; l'argument habituel du bicomplexe donne alors
l'équilibre.\footnote{L'exactitude des sommes est ajoutée : la page ne l'énonce
pas dans la proposition, et la démonstration en a besoin deux fois, pour
passer des objets $a \otimes_{k} x$ à leurs sommes infinies et des
représentables de $(P^{\circ})^{k}$ aux projectifs. La deuxième des quatre
hypothèses que la page énumère sur $N$ ne se lit qu'en partie --- « Une
petite », puis trois mots illisibles, puis « mono » ; qu'elle soit cette condition est possible, mais c'est une
reconstruction de l'édition, que la page ne permet pas de vérifier. Les deux
démonstrations sont les nôtres. La page énonce
aussi, en tête, que le produit dérivé se définit par résolutions projectives en
$F$ et qu'un quasi-isomorphisme en $L'$ en donne un ; et, dans la même liste
d'hypothèses sur $N$, elle nomme la quatrième « $k$-plat ». Elle écrit à deux reprises « $M \mapsto M \otimes_{k} a$ »
