...
% k-additive P, identifies Kar P with « Ul Proj »(P^k) -- a subcategory of the
% projectives of P^k, the two letters read « Ul » being unidentified; Kar P is
% not all of Proj(P^k), since an infinite sum of representables is projective
% and not in it -- and asks which abelian categories are of the form P^k — the answer being the abelian topoi
...
%
% A lower bound, not a date. The print stamp 16 SEP 82 is on the listings of
% pages 4 and 12 (seen on the facsimile, 2026-09-23), and page 12's footer
% announces the machine's shutdown for Friday 17 September; the listing of
% pages 1-2 carries no stamp. What is written on page 12, and on page 3 if it
% is the back of page 4's listing, is therefore no earlier than mid-September
% 1982; nothing bounds any leaf from above, and nothing dates the other
% leaves. That is presumably how the archivists arrived at
...
% — header: « identifies Kar P with the projectives of P^k » now keeps the
%   page's « Ul » qualifier (Kar P is only part of Proj(P^k));
% — header: « The versos date the folder » replaced by a lower bound for the
%   stamped leaves; the stamp, re-checked on the facsimile, is on pages 4 and
%   12, not on the listing of pages 1-2 as the header said;
% — page 11: a note flags the variance of « Tor_i(L, F) (L ∈ P^k) », which the
%   page does write (checked at 800 dpi) and which pairs two contravariant
%   functors;
% — page 12: the words written under the line, earlier read « relativement
%   projectives », read « résolutions projectives » at 800 dpi (same letter
%   shapes as « résolutions » two lines above); « et » before F_i, not on the
%   page, is now marked as added.
...
On identifie\supplied{ra} parfois $L \otimes_k x$ à son image dans
...
projectifs, alors $P^{k}_{M}$ a assez de projectifs\supplied{ $F$}, et ceux-ci
annulent les $\operatorname{Tor}_i(L, F)$ ($L \in P^{k}$).

\note{le $F$ est inséré au-dessus de « projectifs », par un signe de renvoi}

\note{la page écrit bien $L \in P^{k}$. Un tel $L$ est, comme $F$,
contravariant sur $P$, alors que le produit $*_k$ que ces Tor dérivent apparie
$F$ avec un objet de $P^{\circ k}$, comme le $L'$ de la formule ci-dessus et
le $L'$ de $Q^{k}$ au feuillet 12 ; la variance n'est pas réparée ici}
...
\begin{itemize}
  ...
  \item[b)] Si de plus, \add{pour} tout objet projectif \struck{\ill{}} $x$ de
    ...
    redéfini par résolutions projectives $L'_i$ de $L'$\,! \add{et}
    résolutions projectives $F_i$ de $F$. \struck{De plus} \struck{$\ill{} \Phi
    \circledast_k L'$}
\end{itemize}

\note{ici encore la formule écrit $a$ là où la ligne parle de $x$. Les mots
« résolutions projectives » qui précèdent $F_i$ sont écrits sous la ligne,
