...
% 1-8 (kept as (n)) and a recapitulation by roman I, II, III; his
...
\page{1}
\note{p.~1 de l'auteur}
(1) Soit $\overline{\mathbb{Q}}$ la clôture algébrique de $\mathbb{Q}$ dans
...
(2) Supposons $\mathfrak{X}$ connexe $\neq \emptyset$, \add{et}
...
$\mathcal{G}_{\mathfrak{X}}(\xi)$ est un s-groupe ouvert…). On trouve alors,
...
(3) Supposons $\mathfrak{X}$ de t.f.\ sur $\overline{\mathbb{Q}}$.
...
ne se lit pas}
[et à l'image de $\varphi_{\xi}$ dans un $H^1$ convenable…]]
...
(5) Soit enfin, par dessus le marché, une partie
...
(6) \textbf{Récapitulons.} On a donc \struck{$\mathfrak{X}$ schéma
...
($\Gamma' \subset \Gamma$ s-gp ouvert).
\marginal{en marge gauche, au crayon, en oblique, près de (2) : « pour mémoire :
relation avec \uncertain{variétés} \ill{} \ill{} \ill{}… »}

\textbf{Choisissons} (cf.\ normalisation plus bas)

(III) \textbf{un rev.\ universel} $\widetilde{\mathfrak{X}}'$
...
\ill{} $\mathfrak{X}(\overline{\mathbb{Q}})$ \ill{} \ill{}… »}
...
sinon il faut revoir avec plus de soin la situation… »}
\[
...
\ill{} $\mathcal{G}'$ \ill{} et \ill{} \ill{} $\Sigma'$… »}
...
\uncertain{prendre} $\mathfrak{X}$ projectif, \ill{} pour simplifier… »}

\emph{NB} Comme on a les éléments de structure énumérés pour
$(\mathfrak{X}, D, J)$, on les a aussi \uncertain{automatiquement} pour
\marginal{interligne : « étale »} tout rev.\ fini de $\mathfrak{X}'$…\ Il
...
\page{12}
\emph{RemB} Soit $\Gamma'$ un s-groupe ouvert de $\Gamma$, correspondant à
...
\emph{NB} Si $\widetilde{\xi} \in \widetilde{\mathfrak{X}}'(J)$,
...
opération \emph{libre} de $\pi_1 \subset \Sigma'$ sur
$\widetilde{\mathfrak{X}}'(J)$ (ayant $J$ comme quotient)…
...
Ils forment une \emph{catégorie} $C$.
...
\page{17}
\note{p.~9 de l'auteur}
\emph{Définition de $\alpha$}. On va définir
\[
...
\emph{discrète} sur $\Pi$ définie par \struck{\ill{}} \uncertain{un}
foncteur $F : \Pi^{\circ} \to \mathrm{Ens}$, $F(j) = \mathfrak{Z}_j$, …,
...
\page{18}
\emph{Définition de $\beta$}. On définit
\[
...
$\Sigma$ sur $\mathfrak{Z}$…

Variantes profinies…

(8) Considérons, avec notations de (7),
...
l'opération de $\Sigma$ sur $\pi$ par autom.\ intérieurs…\ Pour
...
\[
  J = \mathcal{T}/\pi
\]
sur lequel \marginal{interligne : « et $=$ …\ $G$ »} $\Sigma/\pi$
...
\uncertain{supposons} \emph{$T$ commutatif}
