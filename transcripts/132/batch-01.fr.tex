...
$\mathrm{Im}\, \pi_1(X, x_0) = G$ par chgt.\ de l'arc dans $X$…, et
\uncertain{mod.\ de même} un élément de $\mathrm{Im}\, \pi_1(Y, y_0)$ en
\emph{changeant} le point $z'_0$…]
\marginal{\uncertain{donne} une description \ill{} $(X, Y)$…}\note{note
...
Supposons $\mathcal{C}$ connexe, non vide\supplied{,} $\mathcal{D}$ non
...
Dans le cas de la sphère à 2 trous \supplied{(}i.e.\ le cylindre\supplied{)}, on a $G \simeq \mathbb{Z}$,
...
Cel\supplied{a} \struck{\uncertain{montre}} \uncertain{met} en évidence de façon
...
…\ de l'ext. » est écrit sur une surcharge}
...
\note{à droite de « Soit $\widetilde{G}$ …\ par $\widetilde{H} \to H$ »,
...
\struck{\ill{}} $\widetilde{G}_0$ (via $G$…
...
\to L_{g_i, n_i})$ » ; il est remplacé par « …, \ill{} ce qui est dit
...
\]
Comme $\pi$ discret\supplied{,} $\widetilde{H}_0$ est simplement connexe,
...
« … de $H$ » ; le « $\mathfrak{G}$- » de « $\mathfrak{G}$-hom. » est
...
\uncertain{on} \uncertain{a} …\ $\to \pi_0 G \to \pi_0 G \to 1$}
...
\supplied{(}de $H$ par $N$\add{)} associée à l'ext.\ $\widetilde{G}_0$ de $\mathfrak{G}$ par $N$ et le
