...
L'ensemble \struck{critiques, ou pts} quotient \supplied{appelé l'}ens. des \emph{points} \struck{(de branchement)} de la forme.
...
\page{10}
Un modèle est dit \emph{modèle-segment} si $\exists\, I \in S$ tel que $\widetilde{I} = \mathcal{L}$. \struck{Alors} [Alors $\mathcal{L}_r = \widetilde{I}^{\circ} = \mathcal{L} \smallsetminus \partial I$.] \struck{\ill{}} Cet $I$ est unique. Choisissons un $x_0 \in \partial I$, soit $x_1$ l'autre élément de $\partial I$. Je vais définir une relation \supplied{d'}ordre sur $\mathcal{L}$. \struck{Je dis que} \struck{\ill{}} pour
