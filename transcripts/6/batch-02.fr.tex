...
tal. Au moment de rédiger 1.10.\ il m'avait semblé\supplied{ que}, sans mettre du
...
cas-là où il soit bien raisonnable de parler de \supplied{bi-}cristaux et non
seulement de bi-isocristaux. De façon précise, $\mathrm{DR}^1(f)$ a dans ce
cas une structure de bicristal, défini par les $F_p$ précédents\supplied{,} et des
...
avec le changement de base\supplied{, et définissable sans condition affine sur $S$
...
rang $g$ canonique de $M\otimes_W k=H^1_{\mathrm{DR}}(A)$, \supplied{(}correspondant
à la filtration canonique de cette cohomologie\supplied{)}. Plus généralement et
...
connexion de Gauss-Manin\supplied{)}. En effet, appliquant une telle hypothèse au
schéma modulaire $S$ sur $\underline{Z}_p$ précédent, dont on enlèverait les
points de Hasse nul d'abord\supplied{, d'où $S'$}, on conclurait sauf erreur que
...
\underline{t}_{\varnothing^*}.)$} \supplied{(}Tout ceci est suggéré par les
...
groupe $p$-divisible torique\supplied{, et $S$ réduit à un point (?).}
\supplied{(}Dans le cas général, il semble intéressant d'étudier les
...
le groupe formel\supplied{,} réduit a la structure triviale \struck{\ill{}}
