...
t'exposer\supplied{,} pour me clarifier les idées.
...
catégorie des \supplied{$R$-}préschémas \add{$T$} sous $S$, \supplied{(}i.e. munis d'un
...
relatifs etc\supplied{,} sur $S$ relativement à $R$. Quand
...
défini par $k$, si $\mathrm{car}.k=0$, c'est $k$ lui-même\supplied{;} \add{dans ce
...
Dans le cas envisagé, le foncteur \supplied{« }valeur en $S$\add{ »}, sur la
...
\struck{d'ailleurs} équivalentes\supplied{,} par le foncteur « valeur en
...
\]
Ainsi un \supplied{contra-}foncteur cartésien entre catégories fibrées sur
...
louable vertu d'être stable par produit tensoriel\supplied{ et passage au dual.}
...
\add{$K$} \struck{\ill{}} de $W$. Donc un bi\supplied{iso}cristal (de type fini) sur
...
$W$ que la catégorie $C$ introduite dans 1.0.\ ne change pas\supplied{,}
essentiellement\supplied{,} quand on remplace $R=\operatorname{Spec}(\underline{Z})$
...
libre. De plus comme ici $K$ est de caractéristique nulle\supplied{ et $X_K$ lisse
...
\add{à un cristal} qui est l'image inverse d'un cristal sur $k$, \supplied{(}lui-même
défini par un module de type fini sur $W$\add{)}.\note{« à un cristal » à
l'encre au-dessus d'un mot biffé à la machine ; les parenthèses sont de sa
main.} De ceci et de la rigidité de la notion de cristal on déduit\supplied{,} par
exemple\supplied{,} facilement que tout $p$-cristal cohérent sur $S$ ou tout
...
torsion correspond\supplied{ent} dans la description ci-dessus aux Modules
...
être vrais\supplied{,} sans hypothèse du genre lissité sur $S$.\note{les ajouts de
...
envisagé qu'il définit ne dépend pas\supplied{,} essentiellement\supplied{,} des choix
faits.
...
\supplied{$R$-}flèches $S\to T$ qui \ldots, il faut prendre la catégorie des
...
canulars de détail dans ces notes, mais\supplied{,} je pense\supplied{,} sans
conséquence !
...
$C$, avec\supplied{,} pour toute flèche $(U\to T)\to(U'\to T')$ de $C$\add{,} un
...
c'est vérifié en tous cas (par voie transcendante !) si $S$ est lisse\supplied{,}
...
s'exprime comme un accouplement\supplied{,} définissant une autodualité\supplied{,}
\[
...
2.3. \emph{La cohomologie de De Rham comme bi\supplied{iso}cristal.} A supposer
