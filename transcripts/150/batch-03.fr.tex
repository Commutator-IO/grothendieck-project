...
(c)\note{« c » cerclé.} L'application $\Sigma^{*}(1)\xrightarrow{\ \mathrm{source}\ }\Sigma^{*}(0)$ est \add{propre et} fibrante (« condition \supplied{d'}équisingularité »), cela implique que si $d\subset d'$ est une inclusion d'ens.\ finis tot.\ ordonnés, telle que $d$ soit \emph{segment initial} de $d'$, alors $\Sigma^{*}(d')\to\Sigma^{*}(d)$ est fibrante.
