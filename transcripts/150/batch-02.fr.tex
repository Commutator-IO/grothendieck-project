...
\emph{Prop.} \supplied{(}Si $(X,Y,\mathcal T)$ est décrit par $(\tilde X,\dot X,\mathcal E_{\dot X},Y,\dot X\to Y)$\add{)}, alors les \add{couples} $(X_0,\mathcal E)$ \add{germes de} structures cylindriques compatibles avec le germe de structure cônique $\mathcal T$ correspondent aux \struck{couples} \add{systèmes}
\[
