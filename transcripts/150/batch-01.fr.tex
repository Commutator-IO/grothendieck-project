...
$X$ espace topologique $\Rightarrow$ \add{pré}ordonné par spécialisation : $\mathrm{Spéc}(X)$\note{lecture du nom : « S\uncertain{péc}(X) », les lettres sont surchargées.}
\[
...
$I$ ens. \add{pré}ordonné $\Rightarrow$ espace top., fermés les $A\subset I$ t.q. $x\in A$, $y\le x\Rightarrow y\in A$ ; ouverts les ens. fermés dans $I^{\circ}$ (opposé à $I$) ; base d'ouverts : les ens $I_{\ge x}=\widetilde{\lbrace x\rbrace}$ (\uncertain{ou} désigne l'adh. pour $I^{\circ}$)
...
b) $\exists$ \struck{une relation} \add{\uncertain{pré}}ordre sur $I$, telle que l'on ait $\forall i\in I$
\[
