...
\end{aligned}
\]
et supposons que $\underline s : \Sigma(1)\to\Sigma(0)$ soit \emph{fibrant}. Je dis qu'alors $\pi_0(\Sigma(0))=I(\underline{\Sigma})$ est \emph{un ens.\ \supplied{pré}ordonné}, en prenant comme graphe de la relation d'ordre l'image de
\[
...
Ceci posé, considérons la catégorie top.\ discrète associée à l'ens.\ \supplied{pré}ordonné $I$ \add{$=\pi_0(\Sigma_0)$}, que par abus de langage \add{\ill{}} on désigne par $I$. On a alors un foncteur \add{canonique} de catégories topologiques \struck{canoniques}
\[
...
\emph{propriété universelle} évidente pour les morphismes de \struck{la} catégories top.\ $\underline{\Sigma}$ dans un ens.\ \supplied{pré}ordonné.
...
On désigne par $\mathcal V_i$ \struck{\ill{}} \supplied{pro}voisinage tubulaire (au sens des provoisinages) de $X_i^{*}=X_i\setminus\bigcup_{j<i}X_j$ dans $X$. Ainsi, \struck{\ill{}}
\[
...
X_J^{*}\subset X_{J'}^{*}
\]
et on prendra le \supplied{pro}voisinage tubulaire de $X_J^{*}$ dans $X_{J'}^{*}$, on trouve
\[
