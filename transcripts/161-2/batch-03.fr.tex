...
\begin{enumerate}
  \item[11)] \emph{Corps} $=$ \struck{\ill{}} Anneaux réduits (local de
...
\page{42}
\begin{enumerate}
  \item[12)] \emph{Anneau compact} (i.e. à spectre compact) —
    $X_{\mathrm{cpct}} \simeq \tilde{R}_{\mathrm{top\,cpct}}$
    ...
    (attention, ceci une somme \emph{dans $R$}, pas $\hat{R}$) la famille
  ...
  \item[13)] \emph{Anneau compact réduit} $=$ pseudo\struck{corps}
    ...
    \]
  \item[14)] \emph{Corps sép. clos} $=$ corps str. hensélien
    \[
    ...
    \]
    et $R_{\mathrm{cons}}$ comme dans 11), avec top. \emph{étale}.
\end{enumerate}

\page{43}
\begin{enumerate}
  \item[15)] \emph{Corps alg. clos} $=$ corps \struck{\ill{}} ultralocal
    \[
    ...
    finies surjectives.
  \item[16)] \emph{Anneau \struck{\ill{}} annulé par $n$}
    ...
    \emph{Ex} Si $n$ premier, \ill{} \ill{} les anneaux \ill{}
    \uncertain{corps}.
...
\emph{Ex}\quad $X_{\mathbb{Z}/n\mathbb{Z}\text{-alg}}$,
...
\emph{ouverts} du topos classifiant $\hat{R}$.
...
\page{46}
\begin{enumerate}
  \item[17)] \emph{Anneau parfait de car. $p$} ($p > 0$)
    \[
  ...
  \item[19)] \emph{Corps parfait} \struck{\ill{}} \add{(car $p > 0$)}
    \[
...
\begin{enumerate}
  \item[20)] \emph{Anneau à car.}
    ...
    une $\varprojlim$ \emph{infinie} \ill{}, donc ne donne pas
    ...
    \marginal{\emph{Topos non cohérent} — encerclé, avec un croquis :
    ...
    \emph{localement} par $B$ \ill{} $T' = (\varepsilon \to
...
\ill{} en sens ouvert. \emph{Le \ill{}} : Pour que le Pb \struck{\ill{}
...
\emph{Dans} le cas envisagé $W = X_{\mathrm{car}\,0}$, \struck{\ill{}}
...
\emph{l'anneau nul} \ill{}. Donc \emph{OK}.
...
\begin{enumerate}
  \item[21)] \emph{Anneaux à car. avec propriétés précisées}
    \struck{\ill{}} Re \emph{N.B.}\ On peut reprendre maintenant toutes
    ...
    caractéristique $p$}
  \item[22)] \emph{Anneaux quasi\struck{\ill{}} connexes} \add{de rang
    ...
    $i_{n,j}$ un épi. Comment avec l'objet de base \emph{dans
    $\varepsilon$}
\end{enumerate}
...
e_{\varepsilon}$ est un \emph{iso}, i.e. $\Pi'_{n} \to e_{\varepsilon}$
...
\ill{} gens des \ill{} de \emph{plus} de rapport tout. Cependant le
...
\emph{\uncertain{lexact}} \ill{} (\ill{} à support \struck{\ill{}}
tout). \struck{\ill{}} $e_{i}$ faisceaux des choix \add{de}
pseudo-torseur \add{sous \ill{}} sous le \struck{\ill{}} $A$ \add{est}
\emph{connexe} \add{\ill{}} typiques. \ill{} définition, \ill{}. Je ne
...
\emph{connexes} \ill{} à \ill{} \ill{} pour tt $A_{i}$ \ill{} sur $P$,
...
$M$ \emph{plat} cela est-il une propriété typique [elle est stable par
...
\marginal{$M$ \emph{fid plat} est-ce typique ? — encerclé}
...
\struck{Les objets} ; $E \to F$ hom. de champs \emph{constants} ;
$M \mapsto X$ ; d'où $A_{F}$ ; $R$.

\emph{Algèbres} [de t.f. / p.f. / lisses / étales / nettes] pas typiques
\ill{} \ill{} aux \ill{}

\begin{itemize}
  \item \emph{radicielles} / monomorphiques \struck{\ill{}} OK
    (prod. \ill{}) ;
  \item \emph{surjectives} ?! ;
...
Considérons une Algèbre qui est déjà de \emph{p.f.} On veut « le
vérifier » (à laquelle lisse etc.). Cela se fait-il pour un sous-topos ??

\page{52}
\struck{\emph{Propriétés relatives d'anneaux}}

propriétés \emph{non typiques} (pour topos) (absolues pour anneaux)
\begin{itemize}
  \item artinien, noethérien, de t.f. sur $\mathbb{Z}$ (ou sur $\mathbb{Q}$,
    \struck{\ill{}} ou $\mathbb{Z}_{S^{-1}}$ …) ;
  \item $n$ composantes connexes ;
  \item $n$ composantes irréductibles.
\end{itemize}

Anneaux plongeables dans des corps : il \emph{faut} qu'ils soient
intègres, mais ce n'est pas suffisant \add{\ill{} s'il \ill{} locale
hypothèse}. Pour l'anneau intègre \emph{universel}, \add{(voir anneau
...
\emph{Th.}\ $E$ une catégorie [stable par petites $\varinjlim$ ?
...
\emph{Cor.}\ Soient $E$ et $F$ deux catégories accessibles,
...
\emph{Corollaire}\ \struck{Supposons $f$ $\pi$-accessible. Alors
...
\emph{Th}\ Soit $\mathcal{C}$ une catégorie accessible \add{\struck{\ill{}}
Alors} :
\begin{itemize}
  ...
  \item[b)] les foncteurs covariants \add{s'il est} \struck{\ill{} $\pi$ est
    ...
    $\varprojlim$) et \emph{accessible} \add{stable par petites lim}.
\end{itemize}
\marginal{8.13.2 (+ 8.12.8.1)}

\emph{Corollaire}\ Soient $f : \mathcal{C} \to \mathcal{C}'$ un
foncteur, avec $\mathcal{C}$ accessible \add{petites} :
\begin{itemize}
  \item[a)] $f$ admet un adjoint : dont ssi $f$ commute aux
    \emph{$\varprojlim$} \add{petites} ;
  \item[b)] $f$ admet un \supplied{pro-}adjoint \struck{\ill{}} (resp. un adjoint
    \ill{}) ssi il est exact : \ill{} commute aux \add{petites}
    $\varprojlim$ (resp.) et est \emph{accessible}.
\end{itemize}

\emph{Th.}\ a) Soit $E \to B$ une catégorie fibrée, avec $B$ équiv. à
...
sous-catégorie génératrice \add{\emph{str.}} \struck{telle que} dont
...
$\mathrm{Ind}_{\pi}(\mathcal{C})$ avec $\mathcal{C}$ petite … — 9.6}

\emph{Prop}\ \struck{\ill{}} $\underline{\mathrm{Hom}}(E, F)_{\pi}$,
...
\emph{Cor 1}\ Cas accessible

\emph{Cor 2}\ \struck{\ill{}} Si $F$ est (resp) \struck{\ill{}}
\supplied{$\pi$-}accessible, \struck{\ill{}} pour tt $\pi' \geqslant \pi$,
$E_{\pi'}$ est stable par lim de \struck{type} $I$, avec card $I \leqslant
\pi'$. [Les objets \ill{} stables par \add{petites} $\varinjlim$]

\emph{Cor 3}\ Si \struck{accessible} $E$ est accessible \add{$F$
stable}, tout objet de $E$ est accessible.

Réciproque partielle : \emph{Prop}\ \struck{\ill{}} $E$ catégorie
...
\emph{Cor}\ $E$ catégorie stable par petites $\varinjlim$, \struck{et
...
\emph{Ex.}\ Dans les topos, \struck{\ill{}} \add{à $\varinjlim$
...
\emph{Filtrations cardinales} d'une catégorie $E$ \struck{\ill{}} p. ex.
...
\begin{itemize}
  ...
  \item[c')] \struck{\ill{}} (Si $E$ stable par \add{petites} $\varinjlim$
    ...
    p. ex. $\mathcal{C}$ stable par \emph{lim} finies et $\varphi$
  ...
  \item[h)] \struck{\ill{}} (Si $E$ un topos, $\mathcal{C}$ stable par
    ...
    un \emph{sous-topos} de $\hat{\mathcal{C}}$.
  \item[] \struck{i) \ill{}}
\end{itemize}

\page{58}
$E$ \emph{catégorie}. Conditions équivalentes
\begin{itemize}
  \item[a)] \struck{Il existe une} $E$ est stable par petites $\varinjlim$ et
    admet une petite \supplied{sous-}catégorie pleine str. génératrice
...
à g. ssi il est \emph{accessible} et commute aux \add{petites}
$\varprojlim$]

\emph{Question}\ Une catégorie satisfaisant les conditions a) est-elle
\emph{accessible}, i.e. ses éléments sont-ils accessibles ? Comme, sous
...
\begin{itemize}
  ...
  \item[b)] $T(\mathrm{Ens})$, $T$ une structure algébrique définissable par
    \emph{petites} $\varprojlim$
  \item[c)] $T(\mathcal{C})$, où $\mathbf{T}$ est comme dans b), et
    $\mathcal{C}$ une catégorie \emph{accessible}
\end{itemize}
\struck{\ill{}}

Conséquence : 1) Si $T \to T'$ est un hom. de structures algébriques, d'où
$T(\mathcal{C}) \to T'(\mathcal{C})$ foncteur \emph{accessible} entre
...
acc., $T(\mathcal{C}) \to T(\mathcal{C}')$ est \emph{accessible}
commutant aux petites $\varprojlim$, donc $\exists$ adjoint à g. …
...
\page{59}
\emph{Lemme}\ $\mathcal{C}$ une petite catégorie, $\pi$ cardinal, $E =
...
\emph{Réciproquement} : Soit $E$ une \struck{\ill{}} catégorie \add{$\pi$
...
\begin{itemize}
  ...
  \item[(iii)] Pour tt objet $X$ de $E$, la catégorie $E_{\pi/X}$ est filtrante
    et grande devant $\pi$.
\end{itemize}
\emph{N.B.}\ La condition (iii) est \struck{vérifiée} \add{conséquence de
...
\page{60}
\emph{Axiome c) des filtrations cardinales}

\emph{Pour $(\mathrm{Fil}^{\pi})_{\pi'}$} :
\[
...
($= \pi''$ ?) (\emph{N.B.}\ si $\pi'' \geqslant 2^{\pi'}$, …)}

\emph{Axiome des filtrations cardinales pour les $(E_{\pi'})$}. On a vu
...
\struck{\emph{Cor} Pour \add{supposons que}} \note{amorce encerclée et
abandonnée}

\emph{Cor}\ Soit $E$ une $\mathcal{U}$-cat. stable par petites
