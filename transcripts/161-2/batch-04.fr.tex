...
\emph{Dém}\ Soit $\pi$ un cardinal \add{infini} tel que les objets de
...
filtrantes dénombrables (disons) …
...
\emph{NB 2}\ Soit $\tilde{I}$ la (petite) sous-catégorie pleine de
...
gauche} est \emph{pleinement fidèle}. Or $T_{\tilde{I}}$ est une
...
\emph{algébrique} :
\begin{enumerate}
  \item[1)] $\exists$ \emph{petite} catégorie $J$, et ensembles
...
\begin{enumerate}
  ...
  \item[(ii)] $R_{\alpha+1}$ se déduit de $R_{\alpha}$ en rendant
    inversibles un \uncertain{petit} ens. de flèches, \emph{et}
...
\begin{enumerate}
  ...
  \item[(3)] \emph{Conditions \struck{\ill{}} d'exactitude
    ...
    soit une équivalence (resp. \add{pl.} fidèle, resp. fidèle ?…).
...
\page{65}
\emph{Existence de $\varprojlim$ et $\varinjlim$ dans $T(C)$}
...
associé à $R' \to R$ « \supplied{$\Delta$-}générateur » (tel que, donc, la
...
\begin{enumerate}
      ...
      \item[2°)] $C \to \hat{C}$ est un $\Delta$-foncteur,
        ...
        \emph{construction} $r \in R$, \add{d'où $F : S \to
...
S \to (\mathrm{Ens})$ [et on connaît alors $T$ sur $\tau^{I}$…], ou
...
\begin{enumerate}
  \item[3°)] On a $S \cong \mathrm{Hom}_{\Delta}(R, (\mathrm{Ens}))
    ...
    \emph{contient} $R^{\circ}$ [car les foncteurs repr. \add{cov.}
    ...
    \emph{dans} $S$. Ainsi $R$ est connu quand on connaît $S$ et
    ...
    (\emph{inductives}) de type $d'$, \struck{et pour tout les
...
\begin{enumerate}
  \item[(1)] $d' = d'' = \emptyset$. \struck{Alors} Prenant $\Delta =
    ...
    qui sont représentables par des \emph{petites} catégories. Les
    $I \to R$ générateurs sont \ill{} tous ens. surjectifs.
  \item[(2)] \emph{Conditions d'exactitudes}. On peut par exemple
...
\begin{enumerate}
  \item[(2)] $d''$ $=$ tous les « petits » diagrammes, \add{$d'' =
    ...
    \emph{Pb 1}\ $I$ une petite catégorie, on a pour $C \in
    \mathrm{Ob}\,\Delta$
    \[
    ...
    les objets de $I$ comme famille \emph{cogénératrice} (donc
    $I^{\circ} \to \tilde{I}^{\circ}$ comme famille génératrice).

    \emph{Pb 2}\ $C \in \mathrm{Ob}\,\Delta$, $\Sigma \subset
    ...
    $\Sigma$ est stable par produits \emph{infinis} de morphismes.
    ...
    Supposons qu'on ait un \supplied{pseudo-}syst. inductif filtrant
...
\page{71}
\emph{Th.}\ Les théories algébriques pour $\Delta$ sont celles qui
sont 2-représentables par des $R \in \mathrm{Ob}\,\Delta$ admettant des
petites familles \add{str.} \emph{cogénératrices}. Pour que
$I \xrightarrow{\ \rho\ } R$ \struck{\ill{}} \note{une ligne biffée} soit
\supplied{$\Delta$-}basique, il f. et s. que ses images soient \supplied{str.
...
$T_{\tilde{I}'}$ est une théorie \emph{algébrique} de base $I$. Or
...
\emph{Remarques}\ \begin{enumerate}
...
\begin{enumerate}
  \item[(3)] Supposons que $R$ ait une petite famille
    \emph{cogénératrice} $I$. Alors les $\mathrm{Hom}_{\Delta}(R,
...
\emph{Cor}\ Les \supplied{2-}théories algébriques définissables par
...
\begin{enumerate}
  ...
  \item[(5)] Soit un foncteur $\varphi : R \to R'$ \add{$S$ et $R$ ont
    ...
    à : \emph{certaines} sous-catégories pleines de $R$, savoir
...
\mathrm{Ens}$ [\emph{NB}\ Cependant, si $C$ est un
\emph{topos}, et si $T$, $T'$ se définissent au \uncertain{st.}
des $\varprojlim$ finies, c'est OK …]

\emph{NB}\ La 2-catégorie des théories algébriques pour $\Delta$
...
\begin{enumerate}
  \item[(3)] $d'$ \struck{toutes les} \add{$d'$} \struck{famille de
    ...
    finis $d'' = \emptyset$, pas de \emph{conditions
    ...
    \emph{Pb 1}\ $\Delta$-Enveloppe \add{$\rho_{\Delta}(I)$} d'une
    ...
    $\varprojlim$ des types $d'$ d'objets de $I$ [\emph{NB}\ $I \to
    \bar{I}$ pl. fid. !]. Je dis que cela marche, \emph{pourvu que}
    ...
    \emph{universel} [c'est une catégorie pleine de \uncertain{foncteurs}
    ...
    \emph{Pb 1'}\ \emph{ou} Pb \emph{auxiliaire}\ Si $C
...
\emph{Pb 2}\ $C \in \Delta$, $\Sigma \subset \mathrm{Fl}$,
construire $C\Sigma^{-1}$ \emph{dans $\Delta$}, et des ens. de
couples de flèches qu'on veut égaliser. \struck{Tout marche bien si}
\struck{quotient} \add{d'où} Soit $\rho(C, \Sigma)$ la
\supplied{$\Delta_{1}$-}catégorie de \add{fractions} de $\rho(C)$, définie
par $\Sigma$, et soit $C'$ la plus petite \supplied{$\Delta$-}sous-catégorie
\add{str. pleine} de $\rho(C, \Sigma)$ \struck{qui} telle que $C \to
\rho(C, \Sigma)$ se factorise par $C'$, \add{i.e. telle que $C'$
contienne la sous-catégorie \emph{pleine} $C\Sigma^{-1}$}. (Soit
...
\mathrm{Hom}_{\mathrm{Cat}}(C', D)$ l'est …).
...
\emph{NB}\ Dans les deux cas envisagés (Pb 1 et Pb 2), la
...
\emph{Th.}\ Les théories algébriques de type $\Delta$ sont celles
...
$\mathrm{Ob}\,I' = I$, d'où $T_{R} \to T_{\tilde{I}'}$ \emph{pl.
...
\supplied{$\Delta$-}foncteur pl. fidèle
\[
...
\emph{Cor\,\uncertain{Main}}\ On peut construire les théories
...
\begin{enumerate}
  \item[4°)] Cas $d' =$ ens. des diagrammes finis, $d'' = \emptyset$,
    pas de \emph{conditions d'exactitude}.
...
\end{enumerate}
    ...
    \emph{Lemme} : \struck{\ill{}} Le foncteur inclusion $\Sigma \to
    S$ définit une \emph{équivalence}
    \[
    ...
    \emph{exacts} $\Sigma \to D$ correspondent aux foncteurs
    ...
    \emph{Corollaire}\ \struck{Pour tout} \add{Soit $X \in S$. Pour
...
\emph{Corollaire}\ Pour qu'une \struck{théorie} $\Delta_{1}$-théorie
...
\emph{Corollaire}\ Le 2-foncteur de restriction : $\Delta_{1}$ des
...
morphismes de prés. finie, ou cohérents, de théories…].

\emph{Question}\ Considérons $\Delta$ défini par $d' =$ ens. des
diagrammes finis \emph{discrets}, $d'' = \emptyset$, pas de
...
\begin{enumerate}
  ...
  \item[(1'')] Catégories \supplied{pré}ordonnées avec Inf finis, foncteurs
    qui y commutent
