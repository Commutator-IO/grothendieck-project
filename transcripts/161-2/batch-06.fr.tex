...
\emph{5.4.}\ L'essentiel \struck{\ill{}} se décompose en les questions
...
\begin{itemize}
  ...
  \item[d')] Soient $(R_{j})_{j \in J}$ une petite famille d'objets de
    ...
    ($I$ considéré comme cat. discrète…), et $R$ est-il
    $\lambda$-engendré par la réunion des images des $\psi_{j}$ ?
\end{itemize}

\ill{} \uncertain{envie} des preuves \emph{5.5.}\ Soit $S \subset
...
réciproque étant évidemment vraie…)

\page{102}
\note{author's p. ⑥}

\emph{5.6.}\ Partons de $R \in \mathrm{Ob}\,\mathrm{Cat}_{\lambda}$,
...
\emph{5.7.}\ Si on laisse tomber $I$, une \struck{\ill{}} catégorie
...
\textbf{[6]} \emph{$\lambda$-variable}. On devrait trouver (pour
...
des conditions moyennant lesquelles (6.1) est pl. fidèle. [\emph{Ex}
...
\note{en marge : « ! || »} $\eta_{\lambda,\lambda'}$
...
\textbf{[7]} \emph{Dualité} : \emph{Opposé} d'un « type »
...
\emph{8.1.}\ Pour tout $C \in \mathrm{Ob}\,\mathrm{Cat}_{\lambda}$,
...
\emph{8.1.1.}\ Plus généralement ($\lambda$ quelconque, sans néc.\
...
tout\supplied{e} \struck{\ill{}} construction $\rho$ rel.\ à $T$, le foncteur
correspondant
\[
...
\emph{8.2.}\ \marginal{(remarque)} Notons que pour \struck{\ill{}}
...
T \to \tau^{I}$ (« base ») \emph{fidèle}, i.e.\ $R =
...
\marginal{\emph{NB} L'hyp.\ « $\hat{C}$ une
...
\emph{8.2.12.}\ Si on veut une car.\ intrinsèque, on peut dire
...
comme ss-catégorie pleine de $\widehat{R^{\circ}}$, \emph{celle-ci
contient $R^{\circ}$} (\add{identifié à} la cat.\ des foncteurs
représentables de $R$ dans $(\mathrm{Ens})$) donc …\
...
i.e.\ des données \ill{} une catégorie fibrée sur $C$. [\emph{NB}
...
[i…\ image inverse de $\mathcal{F}_{0}$ par $O_{E} : E^{\circ} \to
...
\add{et les foncteurs induits} \struck{i.e.\ des foncts ex…}
