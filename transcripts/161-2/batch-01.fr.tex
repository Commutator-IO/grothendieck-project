...
\begin{enumerate}
  \item[a)] $\tau_i : C_i \to C_{i+1}$ est \struck{préexact} \supplied{$\delta_i$-}préexact$_{\delta}$, i.e.
...
$\bar{C} \in \mathrm{Ob}\,\Delta_f$, et qui soit \supplied{$2$-}universel pour ces
...
\begin{enumerate}
  ...
  \item[$a_1$)] Pour tout \struck{\ill{}} \add{$\varphi$} \struck{foncteur}
    ...
    couple de \supplied{$2$-}foncteurs $u', v' : R' \to C_i$, et
...
\begin{enumerate}
  ...
  \item[b)] Soit $I$ un ens. discret, $R \in \mathrm{Ob}\,\Delta$,
    ...
    (i.e. …) \struck{ssi} \struck{disque} \struck{$b(I)$ est génératrice}
...
\begin{enumerate}
  \item[$4^{\circ}$)] On met d'autres axiomes \struck{\ill{}} explicites,
    lesquels : a) foncteurs \emph{continus} $I_1 \to E$, i.e.
    ...
    \emph{$\varprojlim$} $R \to E$, plus la condition de transformer
    certains \struck{diagrammes de} cônes projectifs \ill{} de $R'_2$, le type
    classifiant par \ill{} cônes exacts …
\end{enumerate}
On trouve $I \to R_0 \to R_1 \to R_2 \to R_3 \simeq R$.

\marginal{$\hat{J_1} \to R$ est un foncteur de \emph{localisation}}
...
la \emph{continuité} d'un foncteur $C \to E$ (commutant déjà aux
$\underline{\varprojlim}$ finies) par le fait de transformer les familles
couvrantes en \ill{} familles couvrantes … \struck{\ill{}}
