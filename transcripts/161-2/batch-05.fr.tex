...
une flèche de $\Delta$. \emph{NB}\ $\rho$ défini en termes des
2-foncteurs $\Delta \xrightarrow{\ \tau\ } (\mathrm{Cat})$ i.e. la
2-catégorie cofibrée « fonctorielle » $\tau$ sur $\Delta$…
...
\emph{Conséquence} : Existence de 2-limites inductives dans $\Delta$,
...
petit (si les $A_{i}$ ne sont pas petits…), ce qui n'est guère le cas
...
« génératrices » petites, en un sens convenable…

\emph{NB}\ construction des 2-limites dans $(\mathrm{Cat})$ : elle est
...
\emph{« Théorie » $T$ sur $\Delta$} : 2-catégorie cofibrée sur $\Delta$
...
\ill{}}. \emph{NB}\ Si $T$ est 2-représenté par $R \in
...
\begin{enumerate}
  \item[a)] \emph{Structure vide sur $I$ objets} : correspond à
  ...
  \item[b)] \emph{Structure déduite d'une structure $T$} (avec cat.
    des constructions explicites $B'_{T}$) \emph{en ajoutant}
    ...
    \emph{NB}\ Si \struck{on} $T$ est 2-rep. par $R$, alors $B'_{T}
    \simeq R$, \struck{donc $B'_{T'} \simeq R'$ \ill{} $T'$ rep. par $R'$
    \ill{}} \note{deux lignes biffées} \emph{Re NB}\ Si on a
    foncteur fidèle $T \to \tau^{I}$, le composé $T' \to T \to \tau^{I}$
    aussi.
  \item[c)] \emph{Structure déduite d'une structure $T$} (($T' \to
    T$ pl. fid.)) \emph{en ajoutant des flèches} (nouvelles
...
\emph{NB}\ Si $T$ rep. par \struck{$R$, alors} \add{$R$} et $B'_{T}
...
\emph{Re NB}\ $T' \to T$ fidèle, donc si on a un foncteur fidèle
...
\begin{enumerate}
  \item[d)] [composition de structures] Si on a des théories
    ...
    $B'_{T_{\alpha}}$ dans $\Delta$. \emph{NB}\ Si les $T_{\alpha}$
    rep. par les $B'_{T_{\alpha}}$, alors $T$ rep. par $B'_{T}$.
    \emph{Re NB}\ Si $T_{\alpha} \to \tau^{I_{\alpha}}$ fidèles,
...
\begin{enumerate}
  \item[d')] Si on a des théories $T_{\alpha}$ \struck{fidèles sur
    ...
    \emph{NB}\ Si les $T_{\alpha}$ rep. par les $B'_{T_{\alpha}}$,
    $T$ l'est par $B_{T}$. \emph{Re NB}\ \ill{} \ill{} $T_{\alpha}
...
\begin{enumerate}
  \item[d'')] $\varprojlim$ de théories (sans conditions
    particulières)…\ $\leftarrow$ Il suffirait de le postuler pour
    une cat. d'indices bien ordonnée…
\end{enumerate}

\emph{Remarque 1}\ On ne \uncertain{rencontre} à aucun moment des
...
\emph{Remarque 2}\ Soit réciproquement $R \in \mathrm{Ob}\,\Delta$,
...
\begin{enumerate}
  \item[1°)] $R$ petite. On prend $I = \mathrm{Ob}\,R$, ou
    ...
    [\emph{NB}\ On a appliqué 1 fois par a), une fois par c), deux
...
$R_{\alpha} \xrightarrow{\ \sim\ } R$. \emph{NB}\ Pour un ordinal
...
\emph{Dictionnaire pour $\lambda$ quelc} $[= (\lambda_{1},
\lambda_{2})]$
...
\emph{Dictionnaire} [multiple] $\lambda = (\lambda_{1}, \emptyset)$
...
corps universel est le faisceau structural sur ce topos…
...
\begin{itemize}
  ...
  \item morphismes : foncteurs commutant aux $\varprojlim$ et
    $\varinjlim$ \add{des} type\supplied{s} \ill{} : ces
    $\lambda$-foncteurs
...
\begin{equation}
  ...
  \tag{2.4}
\end{equation}
\note{au-dessus, en petit : « couples $(u, \alpha)$, avec… »}
...
\begin{enumerate}
  \item[{[3]}] Si $I$ est réduit à un élément, $\tau^{I}$ est noté
    ...
    \begin{equation}
      ...
      \tag{3.2}
    \end{equation}
    \emph{cat. des $\lambda$-constructions} pour $T$. On
  ...
  \item[{[4]}] \emph{Théories $\lambda$-représentables} \struck{(3.4)}
    ...
    \emph{catégorie \uncertain{$\lambda$-modelaire}} de $T$ :
    \struck{\ill{}}
    \begin{equation}
...
\emph{NB}\ peut-être faut-il aussi se donner \ill{}
d'égalisation des couples de flèches…\ \note{en bas à gauche :
« \ill{} 2 possibilités \ill{}… »}
...
\emph{Définition 5.1}\ La 2-catégorie des $\lambda$-$I$-types est,
...
\emph{NB}\ On peut en donner une construction transfinie.

\emph{Question 5.1}\ \struck{La 2-catégorie} Les \struck{catégories}
