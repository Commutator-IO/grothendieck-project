...
un \emph{sous-topos} de $\rho_{\Delta}(U_{0})$. Ainsi on trouve, si
...
\page{22}
\textbf{d) \emph{Passage à la limite}}\quad Cela \ill{} comme suit : les
...
\page{23}
\textbf{\emph{Structure représentée par} $B_{G} = \hat{C}$}\quad
$C = \struck{\ill{}}(\cdot\,, G)$. \emph{Relativisation dans un topos.}

\textbf{\emph{Topos produit.}}\quad Remarquons que les
\[
  T(E) \simeq \underline{\mathrm{Hom}}\,\mathrm{top}(E, R)^{\circ}
        \simeq \underline{\mathrm{Hom}}_{\Delta}(R, E)
\]
… ces catégories sont stables par $\varinjlim$ \emph{filtrantes}
...
\emph{Question} : $\forall E$, $T(E)$ admet des \struck{produits fibrés}
...
\textbf{\emph{Sous-topos du topos disons \add{$R$} engendré par un objet
...
\emph{sous-topos} \add{(ouverts)} \ill{} de $C$ ; on trouve les
...
\emph{Sous-topos fermés complémentaires} : p. h. f.
...
\textbf{\emph{Question}}\quad Topos classifiant et sous-topos pour les
...
Soit $\Delta = \Delta_{\sigma,\rho}$ la \supplied{sous-}catégorie de
...
\page{27}
… sur les catégories envisagées pour $\Delta$ (au plus des conditions
d'existence \add{$E$} au lieu dans a), que si les \add{types de}
(\emph{lim} que les formulations des conditions impliquent existant
\emph{tjs} dans les $E$ envisagées, et si les foncteurs admis entre ces
$E$ y commutent…)].
...
\textbf{\emph{Dém.}}\quad On construit par réc. transfinie un
...
\begin{enumerate}
  ...
  \item[c)] $\tau_{i} : C_{i} \to C_{i+1}$ est \emph{préexact} par
...
(On dit que $(\bar{S}, \bar{\lambda})$ est \emph{déduit} de $(S, \lambda)$
en \emph{dédoublant} les sommets des cônes critiques de $S$ ;
...
\emph{unique} extension en $\bar{u} : \bar{S} \to E$ qui soit
...
\begin{enumerate}
  ...
  \item[3\textsuperscript{o}] $f : X \to Y$ \emph{effective}
    $\Longleftrightarrow$ $\mathrm{Hom}(P, X) \to \mathrm{Hom}(P, Y)$
    surjectif \add{(epi)} ;
  \item[4.] \emph{fiber prod} ;
  \item[5\textsuperscript{o}] $P \to P \times K$, $P \times n$,
    $\mathrm{Hom}(P, \Pi)$.
\end{enumerate}

\emph{morphism of \struck{the} \emph{effective} descent.}
...
\[
  ...
  \qquad \mathbb{T} = (UF, -, -) .
\]
\uncertain{Beck}, \uncertain{Barr}, \emph{Linton}. \quad $U$
\emph{tripleable}.
...
\begin{enumerate}
  ...
  \item[3)] \emph{anneau local hensélien} [si $X' \to X$ dans $R$ est
  ...
  \item[5)] \emph{anneau ultralocal} (si $X' \to X$ \add{dans $R$}
    \struck{est} couvrant pour fppf, alors $A(X') \to A(X)$ est épi ; \ill{} et
    dans les cas universels $U = \ill{}$ dans \ill{}. \emph{Question} :
...
7) \emph{anneau intègre} : a) $A \amalg A \xrightarrow{\ u\ } D \subset
...
\page{39}
8) \emph{Anneau irréductible} : $A_{\mathrm{red}}
