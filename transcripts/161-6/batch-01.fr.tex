...
%          paradoxiques? prêtées à Labarde) » — cf. folder 162-1, the lending
%          register. No \page; the tab's words head the file and a \note
%          records the rest.
...
%          Hom_G(G/H, E) = E^H; page 17 is a first draft of the computation of
%          pp. 3-4; page 18 proves G ≃ A_5 via an action on 15 objects
...
\[
\begin{cases}
  \|x(u,v;\alpha) - u\|^2 \underset{\text{par raison de sym.}}{=} \|x(u,v';\alpha) - u\|^2 = (\cos\alpha - 1)^2 + \sin^2\alpha = 2(1 - \cos\alpha) = 4 \sin^2 \frac{\alpha}{2} \\[1ex]
  \|x(u,v,\alpha) - x(u,v',\alpha)\|^2 = \sin^2\alpha \, \|v - v'\|^2 = \sin^2\alpha \; 4 \sin^2 \frac{\theta}{2} = \Bigl(4 \sin^2 \frac{\alpha}{2} \cos^2 \frac{\alpha}{2}\Bigr)\Bigl(4 \sin^2 \frac{\theta}{2}\Bigr)
\end{cases}
\]
...
\end{aligned}
\]
\note{les lettres $A, B, C, D, E, S$ ne sont pas portées sur la figure ; les couples sont apparemment des couples d'arêtes opposées. La seconde égalité de la dernière ligne, $\alpha^{2}(Q)$, se lit ainsi ; on attendrait $\alpha^{3}(P) = P$.}
...
\[
  \frac{1\,2\,3\,4\,\struck{0}}{2,4,1,3,0} \qquad \frac{}{3,1,4,2,0} \qquad \zeta \quad \zeta^{2} \qquad u, 2u, 3u, 4u, 0 \qquad \zeta^{i} \qquad e^{i\theta} \qquad u^{2} + w^{2} = 1 \qquad \theta^{2} + \mu^{2} = 1 \qquad x^{2} - y^{2}
\]
...
\]
\[
  1 + x + x^{2} + x^{3} + x^{4} = 0 \qquad \Bigl(2\sin\frac{\alpha}{2}\Bigr)\Bigl(2\sin^{2}\frac{\pi}{\uncertain{4}}\Bigr) \qquad \alpha = 60\,? \qquad \cos\frac{\alpha}{2} = \frac{\sqrt{3}}{2} \qquad \sqrt{3}\sin\frac{\pi}{5} = 1 \qquad \frac{\pi}{6}
\]
