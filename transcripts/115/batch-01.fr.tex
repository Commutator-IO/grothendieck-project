% Transcription — fonds Alexandre Grothendieck, cote 115, lot 1 (feuillets 1-14).
% Établie sur le fac-similé archives/batches/115/batch-01.pdf (miroir de
% https://grothendieck.umontpellier.fr/115.pdf), selon la skill
% transcribe-grothendieck. Feuillets 4, 6 et 12 : versos de listings
% informatiques sans rapport (datés 02 JUN 82) — absents, comme il se doit.
...
\section*{Correspondances fonctorielles (feuillets 1 à 5)}
...
\leaf{2}
Formules d'adjonction, en tête de feuillet :
\[
...
\]
\note{le système d'indices de la colimite est illisible. Le reste du feuillet
— un cercle de catégories de foncteurs et de petits diagrammes — est un
premier état raturé de la roue du feuillet 5 ; lectures trop incertaines pour
être transcrites}
...
En marge haute : $A^{\circ}, \widehat{A}^{\circ}, A^{\vee}$ ;
$B^{\circ}, \widehat{B}^{\circ}, B^{\vee}$ ;
$\bigl|\widehat{A}^{\circ} \times A^{\vee}\bigr| \times
 \bigl|\widehat{B}^{\circ} \times B^{\vee}\bigr| \to \mathrm{Ens}$
\note{décorations « (4 co) », « (6 (co)) » non élucidées}

La roue des huit catégories de foncteurs autour de
$(A\times B)^{\wedge} = \underline{\mathrm{Hom}}(A^{\circ}, B^{\circ};\,
\mathrm{Ens})$ :

\begin{tikzcd}[column sep=tiny, row sep=scriptsize, nodes={font=\scriptsize}]
& \underline{\mathrm{Hom}}^{!!}(\widehat{A}^{\circ}\times\widehat{B}^{\circ};\ \mathrm{Ens}) & \\
\underline{\mathrm{Hom}}^{!}(\widehat{B}^{\circ},\widehat{A}) & & \underline{\mathrm{Hom}}^{!}(\widehat{A}^{\circ},\widehat{B}) \\
\underline{\mathrm{Hom}}(B^{\circ},\widehat{A}) & (A\times B)^{\wedge} \arrow[uu, "\approx"] \arrow[ul, "\approx"'] \arrow[ur, "\approx"] \arrow[l, "\approx"'] \arrow[r, "\approx"] \arrow[dl, "\approx"'] \arrow[dr, "\approx"] \arrow[dd, "\approx"] & \underline{\mathrm{Hom}}(A^{\circ},\widehat{B}) \\
\underline{\mathrm{Hom}}_{!}(B^{\vee},\widehat{A}) & & \underline{\mathrm{Hom}}_{!}(A^{\vee},\widehat{B}) \\
& \underline{\mathrm{Hom}}_{!!}(A^{\vee}\times B^{\vee};\ \mathrm{Ens}) &
\end{tikzcd}

« toutes les flèches du diagramme \ldots{} sont des équivalences de
catégories »
\note{les huit flèches rayonnent du centre vers l'extérieur, comme sur le
feuillet — le centre est la forme dont chaque nœud est une curryfication. Le
pourtour porte en outre des arcs reliant les nœuds voisins, étiquetés
$(\widehat{A}\to)$, $(\widehat{B}\to)$, $(A^{\vee}\to)$, $(B^{\vee}\to)$,
et les quadrants sont marqués $\wedge\wedge$, $\wedge\vee$, $\vee\vee$,
$\vee\wedge$ ; les deux nœuds de gauche, en partie masqués par une
déchirure, sont de lecture incertaine}
...
\section*{La dualité entre $\widehat{A}$ et $A^{\vee}$ (feuillets 7 à 9)}
...
\leaf{8}
\note{feuillet entier biffé de trois grands traits obliques}
Multivariances :
\[
...
\leaf{9}
\note{page 4 de la main de l'auteur ; le feuillet ne porte que ces deux
formules, encadrées}
\[
...
\section*{L'enveloppe $\widetilde{A}$ (feuillets 10 et 11)}
...
$\widetilde{f} : \widetilde{A} \to \widetilde{B}$ ; en bas du feuillet,
encerclé :
...
\section*{Noyau autodual, recollements, coefficients (feuillets 13 et 14)}
...
\]
\note{à gauche du feuillet, un $\overline{\varepsilon}(A)$ isolé, sous un
trait de séparation}
