...
le sous-schéma en groupes de $G$ qui correspond aux \supplied{$\otimes$-}auto\-mor\-phismes
...
un groupe de Galois $G$, et plus généralement toute \supplied{$\otimes$-}sous-catégorie
...
\struck{nécessairement} \add{évidemment} munis d'une \supplied{pré-}structure de
...
$H^{*}(X, \underline{\mathbb{Q}})$, d'où un \supplied{$\otimes$-}foncteur naturel
...
motivique (associé au \supplied{$\otimes$-}foncteur de Betti $H_{Bet}$) est un
...
la \supplied{$\otimes$-}catégorie de pré-structures de Hodge engendrée par les
...
de Galois d'une \supplied{$\otimes$-}catégorie \add{$\underline{C}$} de
...
\marginal{relations entre loi $\check{X}$ et antipo\supplied{de} données par $U$
resp.\ $A$}
