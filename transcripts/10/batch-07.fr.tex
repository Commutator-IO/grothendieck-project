...
topologiques compacts, avec le foncteur $\otimes$ donné pa\supplied{r le} produit
cartésien, et la donnée évidente d'associativité et de commutati\supplied{vité}
...
de 3º). Remplaçant $k$ par une algèbre quelconque, on trouve
...
les données et conditions de 1º), on doit avoir un foncteur
...
de 1º, \add{avec $k = \operatorname{End}(\underline{1})$ un
...
$F : \underline{C} \to \operatorname{Modf}(k)$, et correspon\supplied{dant} à ce
titre à une algèbre profinie (ass.\ un.) $U$, resp.\ une coalgèbre ass.\ unit.\
$A$, sur le corps $k$ (cf.\ nº 6). On \struck{voit que la donnée
d'} suppose donné\supplied{e} une opération $\otimes$, compatible avec $F$, i.e.\
...
\marginal{Dans le cas du nº 6, \emph{gradué}, on aura
...
\supplied{co}modules sur \struck{\ill{}} $A'$
...
récupérant toujours après coup.\marginal{Mais, cf.\ exemples nº
11}

Il resterait alors la question d'une description \emph{intrinsèque} d'une
catégorie $\underline{C}$ comme dans le nº 1, \add{avec $X$}
...
nº 11, il faudra donc $\Delta_{U}$ [i.e.\ $A$] commutatif, et
...
13) \emph{Variante utile du tapis de nº 6.}\note{Le chiffre tapé
...
nº 6,\marginal{en termes de $\Delta$} et il y a lieu de donner
des variantes graduées (si possible plus générales et astucieuses qu'au
nº 8). En fait, les résultats spécifiques au
nº 6 seraient plutôt, dans le cas d'un foncteur fidèle (et
...
$\underline{C}$, avec l'hom\supplied{omorphisme}
