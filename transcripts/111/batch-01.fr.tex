...
% typed interlinear insertions, and then gone over by hand in ink and, on
% pages 13-14, in pencil. The typed text is headed « Tapis de Quillen. » on
...
% thought, which is then given in a \note{}. Everything in ink or pencil is
...
% « simplicuux », « semi-simplivial », « fancteurs », « dérigés »,
...
% coproduct on page 3; almost every word of the foot note of page 3 after
% « n'est »; the interlinear note of page 7 (« Si on identifie mod ...
% équivalence ... catégorie dérivée des complexes ») and its left-margin
% companion, cancelled; the slanted marginal of page 8 (« non, on a un
% foncteur pl. fidèle (?) ... ess. surj. (?) »); the cancelled second half of
% the handwritten paragraph of page 10; the label of the right-hand arrow of
% the square on page 13; the parenthesis on the generators of Lambda on page
% 13; « Quillen ignore si » on page 13; the long slanted marginal of page 16,
% of which only a few words carry.
%
...
\subsection*{I \quad Relations entre catégories et ensembles semi-simplicuux.}

A toute catégorie $C$, on associe un ensemble semi-simplivial $S(C)$,
...
$\varprojlim$, ou si on préfère, des $H^{i}$ du topos $\widehat{C}$. On voit
...
écrit $\widehat{C}$ à la machine}

Les systêmes de coéfficients (ensemblistes, déf.) sur $C$ (= les foncteurs
...
canoniquement « homotopiquement équivalente à $C$ » i.e.\ isomorphe à $C$ dans
...
local contravariant $F$ (défini classiquement par le complexe cosimplicial des
...
cohomologies (avec cas non commutatif inclus). Supposons que $X$ et $X'$
soient\note{« $X$ et $X'$ » est tapé en interligne au-dessus de « $X$
(resp.\ $X'$) » biffé} \emph{localement homotopiquement triviaux}, i.e.\ que
pour tout entier $n \geq 1$, tout objet $U$ de $X$ ait un recouvrement par des
...
\marginal{Attention, cette condition n'est \uncertain{typiquement} pas
satisfaite par les schémas \uncertain{munis de leur topologie étale} (mais
bien par \struck{\ill{}} les \ill{} loc.\ noeth.\ avec top.\
...
\marginal{Si on \uncertain{identifie} mod \ill{} équivalence, cela correspond à
travailler dans la catégorie \emph{dérivée} des complexes, \ill{} les
invariants $\pi_{0}$, $\pi_{1}$).}\note{deux lignes manuscrites serrées entre
les lignes tapées, qui partent de la marge gauche. Plus haut dans la marge,
...
ponctués de la catégorie homotopique : forment-ils des catégories équivalentes
par le foncteur espace des lacets\add{ ?}).\note{un double trait vertical
manuscrit en marge de la parenthèse}

\marginal{\uncertain{non}, on a un foncteur \uncertain{pl.\ fidèle} (?)
\ill{} \uncertain{ess.\ surj.} (?) \ill{}}\note{note écrite de biais dans la
...
\struck{\ill{} \ill{} \ill{} de l'opération \ill{} \ill{} \ill{} foncteur
\ill{} shift (\ill{} \uncertain{géométrique} \uncertain{évidente} sur les
\ill{}-catégories ...)}}\note{paragraphe manuscrit sous la frappe, appelé
par un signe de renvoi en marge ; sa seconde moitié est barrée de traits
obliques et ne se lit que par mots isolés}
...
$F^{\bullet} : C^{\circ} \to \mathbf{B}$. On vérifie enfin que le couple
...
\begin{tikzcd}
X_{s} \arrow[r, "\alpha_{s}"] \arrow[d, "\alpha_{s}"'] & X \arrow[d, "\Gamma"] \\
...
carré sont portés à la main sur des blancs de la frappe. La flèche de droite
porte à la main un $\Gamma$ suivi d'un indice et d'une parenthèse,
\uncertain{$\Gamma_{p}(i_{X})$}, dont on ne donne que le $\Gamma$}
...
de polynômes connue \add{(à une \uncertain{infinité} de générateurs
$X_{i}$ (\ill{} \ill{}) ...)}.
...
\]
\struck{(compatible avec multiplication)} tel que l'on ait :
\add{(il est douteux qu'il soit)}\note{la main entoure « compatible avec
multiplication » d'une parenthèse et écrit au-dessus « il est douteux qu'il
soit » ; un point d'interrogation dans la marge}
\begin{itemize}
\item[a)] L'homomorphisme correspondant $H^{\bullet}(X)
\otimes_{\mathbf{F}_{2}} \Lambda \simeq B_{\cdot}(X)$ est un isomorphisme (ce
sera vrai pour tout relèvement individuellement ...), et
\item[b)] Le relèvement est compatible aussi avec les hom.\ de Gysin.
\add{\uncertain{Quillen ignore si}}
\end{itemize}
\note{un crochet manuscrit devant b), et un grand trait en arc dans la marge
en regard de a) et b)}
...
associée à $\mathbf{B}$, on trouve donc tous les modules gradués libres de type
...
\marginal{= karoubienne + additive}\note{au crayon, en tête de la page,
...
on trouve maintenant la catégorie des \emph{tous} les modules gradués libres de
...
noethérien, i.e.\ le nombre de générateurs est-il fini ?} \add{\emph{limite}
...
aurait gardé l'équivalence rationnelle, --- qui est bien trop fine beaucoup
d'égards. Pour trouver un équivalent plus étroit, il convinmt de reprendre la
...
$C$ \add{qui opèrent trivialement sur \ill{} $H^{1}(S^{1})$}.\note{après
...
manuscrite finale se termine sur un mot raturé}
...
\marginal{\ill{}, en \ill{} \ill{} \ill{} \ill{} \ill{} : \ill{} \ill{}
d'\uncertain{irréductibles} \ill{} \uncertain{réductif} \ill{} \ill{} :
\uncertain{engendré} \ill{} \ill{} \ill{}}\note{note de sept lignes écrite
de biais dans la marge gauche, en regard de la parenthèse sur $G$, qu'une
flèche y rattache ; seuls quelques mots isolés s'y distinguent}
...
f^{*}(T_{Y})$, en $X \to H$. Les éléments de $H(X, Y)$ seront définis par des
...
en interligne et rattaché à sa place par un trait manuscrit}
