...
% notation paragraph said the batch was « kept in line with batches 1 and 3 »,
% which contradicted batch 3's header (concurrent passes, no earlier .tex) and
% was not so for batch 1 (k_X, not κ); it now says what agrees with which
% batch and that the passes ran concurrently. The transcription is unchanged.
%
...
%
% Notation fixed for the batch: the category of the base \mathcal{V}, its
% opposite \mathcal{V}^{\circ} for Poincaré algebras; the external product
% \boxtimes; the form κ_A (his k_A), distinguished from the ground ring k;
% the unit written U_Y on page 22 kept as $U_{Y}$; graphs Γ_f; ⊗^{ord} for his
% « ⊗ » surmounted by « ord ». The three batches of the folder were transcribed
% on the same day by concurrent passes (batch 3's header says the same), so no
% batch fixed the notation of another. \mathcal{V}, \boxtimes and Γ_f agree
% with batch 1; κ for the form agrees with batch 3, while batch 1 keeps his
% k_X.
%
...
\uncertain{Composées} \ill{} \add{de} telle\supplied{s} applications :
