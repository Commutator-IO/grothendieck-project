...
% \textcircled{n}. His \boxed formulas kept boxed.
%
...
\underline{NB} $f$ (commutant à $G$) connu quand on connaît $f(x)$, si $G$
transitif sur $E$.
...
\underline{Déf} \underline{Graphes}
\[
  \left.
  \begin{array}{l}
...
\underline{Axiomes} 1) $\forall a \in A$, $\operatorname{card} R\{a\} = 2$
...
Un graphe est dit \struck{\ill{}} \underline{strict} si une arête est connue
quand on connaît ses sommets.

\struck{Une} La donnée d'un \underline{graphe strict} équivaut donc à la
...
\struck{Remarque} \underline{Réalisation topologique d'un graphe}
...
vectoriel (de dim finie\dots) sur $\mathbf{R}$ : homéom.\ dans
...
\]
telle que l'image d'une arête soit un \underline{segment}.

\underline{Prop} Pour qu'un graphe fini admette une réalisation
...
$\operatorname{réal}(S, A, R) \hookrightarrow E$ qui prolonge\dots
...
\underline{NB} Tout graphe est de façon unique réunion de graphes connexes
\uncertain{disjoints}
\note{« disjoints » est ajouté sous la ligne, rattaché par un trait}

[graphe somme : $S \amalg S'$, $A \amalg A'$, $R \amalg R'$ \dots ]

$\to$ Graphe (\uncertain{multi}) polygonal (ou \underline{contour}
(combinatoire))
\[
...
\underline{Prop} Un \struck{graphe polyg.}\ contour $\neq \emptyset$ connexe
...
$\mathbf{Z}$ ($c = \operatorname{card} \mathbf{Z}/H$)\dots

\note{dans la marge gauche, deux dessins : au crayon, un contour à deux
sommets et deux arêtes ; à l'encre, un pentagone}

\underline{Orientation d'un contour} : déf.

\underline{Prop.} Soit $X = (S, A, R)$ un contour \struck{\ill{}} connexe
$\neq \emptyset$.

a) Soit \struck{Un} $g$ un autom.\ $g$ de $X$. \underline{Conditions équiv.}
\struck{\ill{}} :
\begin{itemize}
...
\page{6}
\underline{Contour} \underline{orienté} « revient au $\equiv$ » que de se
...
\page{7}
\underline{Orientation d'un contour.}
...
\underline{Prop} Si $(S, A, R)$ est un contour connexe, il a exactement
\underline{deux} orientations ($\omega$ et $-\omega$).
...
\struck{Prop} \underline{Prop} Soit $\omega$ orientation des contours
$(S, A, R)$.
...
1) \underline{Ens.\ avec permutation} $(E, u)$ $\Leftrightarrow$
$\mathbf{Z}$-ens.
...
\underline{Cor} Les classes d'éléments de $\mathfrak{S}_n$ mod.\ conjugaison
...
2) \underline{Rectification} Les contours orientés correspondent aux ens.\ à
permutation $(E, u)$ tels que $u$ n'ait pas de \underline{pt fixe} (car ce ne
...
\page{9}
\underline{Ex} Groupe \uncertain{affine} \struck{\ill{}} :
...
\]
\underline{NB} Si $E$ est un torseur sous $G$, et $E \neq \emptyset$, alors
...
\underline{Corollaire} Si $n$ impair, $t \mapsto \sigma_t$ est une bijection
...
\underline{NB} Si $n$ pair, elle n'est pas bijective, \struck{mais} ses
fibres sont toutes de cardinal $2$\dots

\struck{\ill{}} Sommet \underline{opposé}, symétrie canonique
...
\underline{Parenthèse} Produits fibrés d'un
...
\underline{Remarque} Toute extension splittée $G$ de $\Gamma$ par $Z$ provient
par image inverse d'une ext.\ splittée $Z.\operatorname{Aut}(Z)$.

4) \uncertain{Ens.}\ de petit cardinal

a) \underline{Cardinal 2} \quad Ens de cardinal $2 \longleftrightarrow$ torseurs
sous $\mathbf{Z}_2$ (ou encore, \underline{espace affine sur $\mathbf{F}_2$}
...
couple $\Gamma \subset \mathfrak{S}_E$ ss-groupe, tel que \dots\ ;
\uncertain{muni} par ens.\ à groupe d'op.\ \underline{fidèle}\dots

Une structure d'espace \underline{affine sur $k$} est alors la donnée d'un tel
...
\page{14}
\underline{Exercice} Réalisation régulière d'un polygone.

Soit $E$ plan affine euclidien sur $\mathbf{R}$ (i.e.\ muni d'une dist\ldots\
sur le plan $V$ des translations). Une réalisation rectiligne d'un polygone
$(S, A, R)$ est dite \underline{régulière} si tous les côtés du polygone ont
...
\uncertain{c)} \underline{Cas} Notion d'enveloppe affine (ou convexe, de l'ens.\
...
transitivité groupe des similitudes\dots) Existence, unicité modulo
...
Notion d'enveloppe affine (ou vectorielle) d'un polygone combinatoire\dots\
\uncertain{sans} précision

\underline{Fonctorialité de $X \times_G Y$ par : $(G, X, Y)$\dots}
\[
...
\[
  ...
  \text{ : droite}
\]
\underline{NB} Si $G$ opère fidèlement sur $E$, $G$ opère « sans pts fixes »
...
torseurs\dots\ Dans ce cas (et dans ce cas seulement)
...
$(G, E)$-structure \ill{} équivaut à celle d'un \underline{torseur} sous
...
\underline{Applications} \struck{\ill{}} \textcircled{1} Yoga : structures isomorphes : \uncertain{les}
...
\page{17}
\textcircled{2} Donnée d'un $G$-torseur $T$ $\longleftrightarrow$ donnée d'un
...
\textcircled{1} car $G = \operatorname{Aut}_{\mathcal{C}}(E)$, $\mathcal{C}$
...
\textcircled{3} Soit $\mathcal{C}$ une espèce de structures (sur un ens.\ de
base, disons), $E$ une structure de type $\mathcal{C}$,
$G = \operatorname{Aut}_{\mathcal{C}}(E)$. Alors, pour un ens.\ $F$, la donnée
d'une structure de type $\mathcal{C}$ \underline{isom}\add{orphe} à $E$
\underline{équivaut} à celle d'une $(G, E)$-structure. De cette façon,
\textcircled{1} devient un particulier de \textcircled{2}.
...
\]
(il faut prendre $n \geq 3$ pour que ceci soit une réalisation rectiligne\dots\
!)
...
\marginal{\underline{cf}\ le groupoïde connexe, $E \in \operatorname{Ob}
...
\textcircled{5} \struck{Changement de} Extension du groupe d'opérateurs
...
\underline{NB} Si $F$ un $G'$-ens.\ à droite, dans
\[
  \left\{
  \begin{array}{l}
...
\page{20}
\underline{via} réduction d'un groupe $G$ ; il suffit de tordre par le
torseur ${}^{G'}T = T'$. (Ex : si $F$ muni de quelques structures,
\struck{diverses} \ill{} on prend $G' = \operatorname{Aut}(F)^{\circ}$\dots\
et $F^{T}$ \ill{} \ill{} que la structure \uncertain{déduite} sur
$G'$-torseur $T'$\dots)
\note{« quelques » est ajouté au-dessus de la ligne ; « déduite » n'est pas
sûr}

\textcircled{6} Opérations \struck{sur les} du produit sur les torseurs sous
groupe \underline{commutatif}. Opération à droite \uncertain{produit}
\[
...
\underline{Prop} Un produit de torseurs est un \underline{torseur}.

\struck{\ill{}}

\underline{Prop} $\forall$ torseur $X$, $\exists$ torseur \struck{$Y$} $Y$ et
iso.\ $X \wedge Y \simeq \underline{G}$. \uncertain{Pour} \uncertain{un} \uncertain{tel}
$Y'$, $\exists$ iso.\ unique $Y \simeq Y'$ tel que\dots
\[
  Y \simeq X \times_G (G \xrightarrow{g \mapsto -g} G)
\]

\textcircled{7} \underline{Ex} \struck{Si Orientations, \uncertain{d'un} $X \wedge Y$
...
\struck{\underline{Ex} Soit $B$ ens.\ fini avec \uncertain{rel.}\ \ill{}. $R$}
