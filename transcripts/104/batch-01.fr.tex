...
% rectos — and pages 7, 9, 11, 13 and 15 are the versos of the first five of
% those six leaves and carry no number; the text runs continuously across each pair, so
...
%
% Pages 1 to 5 bear on the same construction, less worked out and from further
% off; nothing on them dates them relative to pages 6 to 16. They are
% transcribed at their rank because they are mathematics, not because they
% are tidy. Page 1 is a row of sketched cell complexes and one
...
% <= F_2 with its s_i, t_i, sigma_i, mu_i^j and three bare letters a, l, r, and the formula F_n = Fl C_1 x ... x Fl C_n x Ob C_{n+1}. Page 4 is
...
% pi_1(f) injective in the margin. Page 5 sets down, in rough, data that
% pages 6 to 9 write out, with the ordered set called I instead of N, and it
% carries the cube count 1 + n.2 + n(n-1)/2 . 2^2 + ... + 2^n = (1+2)^n, which
% page 10's margin states in closed form as Card I_n = 3^n.
%
...
% which has 2n+3 elements. (The Delta_{n-1} x {+-1} of the margin belongs to
% the note beside Ex. 2, the cube, which ends « Card I_n = 3^n »; it says
% nothing about Ex. 3.)
%
% Pass: Opus 5 (claude-opus-5), 2026-09-13 — first pass, unchecked against the pages by a human.
%
% Revised 2026-09-23 (Opus 5.5), after the find-novelty pass, checked against
% the facsimile archives/batches/104/batch-01.pdf (PDF page = folder page):
% — page 9: the note « sa page 3 » removed; page 9 is an unnumbered verso like
%   7, 11, 13, 15 (only a faint offset of page 10's « 3 » shows). Page 10's
%   note, « sa page 3, suite », now reads « sa page 3 ».
% — page 5: the note said the typed page names the D-tilde_n; it names the
%   basic objects D_n (underlined by the typewriter), and the D-tilde_n are
%   his hand's. Corrected.
% — page 10: the margin block beside Ex. 2 re-read in part (« sections »,
%   uncertain, and an arrow Delta_{n-1} x {+-1} --> Delta_{n-1}); the note and
%   this header no longer use its Delta_{n-1} x {+-1} as evidence for Ex. 3.
% — page 3: the subscript of the amalgamated sum flagged uncertain (drawn like
%   a dotted B; the neighbouring F-dots are overwritten glyphs of the same
%   shape); the last factor « x Ob C_q » of F_n, read unstruck, is struck on
%   the page and its index is the 4 of the F_3 line.
% — header: « seen earlier », « the draft of pages 6 to 9 », « will reduce »
%   and « coherence letters » rephrased to what the pages show; the body notes
%   « champs d'essai pour ... au propre » and « rédaction au propre » likewise.
...
\note{le dernier facteur est biffé ; son indice est tracé comme le 4 de la
ligne $F_3$. La suite des formules demanderait $\mathrm{Ob}\,C_{n+1}$}
...
\note{l'indice est tracé comme un $B$ pointé ; les deux $\dot{F}$ de la page
sont des $F$ surchargés d'un $O$, de la même forme, et l'indice peut être ce
même signe, plus petit}
...
\underline{N}$. Le tapuscrit nomme ainsi les objets $D_n$, soulignés à la
machine ; les $\widetilde{D}_n$ qui suivent sont de sa main. C'est pour ce
nom des $D_n$ qu'elle est signalée}
...
\marginal{$I_n = $ ens.\ des \struck{\ill{}} \uncertain{sections} \ill{}
$\Delta_{n-1} \times \{\pm 1\} \to \Delta_{\uncertain{n-1}}$ \ill{}
...
mots de la note de l'Ex. 2 y portent. Le $\Delta_{n-1} \times \{\pm 1\}$ est
dans cette note-là, à côté de l'Ex. 2, et ne porte pas sur l'Ex. 3. Le
« Card $I_n = 2n+1$ » de l'Ex. 3 ne s'accorde pas avec le
...
induit) --- et remarquons\supplied{:} que $f$, bien sûr
...
du moins \supplied{n'a qu'un nombre fini d'}éléments maximaux, et que $K$ est
