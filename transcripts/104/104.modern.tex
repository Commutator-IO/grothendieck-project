...
% Pages 1-5 are trial sheets for the same construction and for the globular
% apparatus of Pursuing Stacks, read briefly.
%
...
%   index n+1 (not q), as the pattern of the formulas requires.
%
...
conditions d'équivalence, un premier état de la construction.
...
Les pages 6 à 16 sont la rédaction au propre, sous sa pagination 1 à 6, portée
sur les rectos seulement, les versos continuant le texte ; elle est continue.
...
huit. Ils ne sont pas rédigés ; on en relève ce qui prépare la suite ou éclaire
le contexte.
...
lettres $a$, $\ell$, $r$ --- les contraintes d'associativité et d'unité à
gauche et à droite d'une structure de dimension 2. Puis des cellules
...
\]
essayées d'abord sur $C_{1} = \Delta_{n}$, $C_{2} = \Delta_{n'}$.\footnote{Trois
indices sont écrits autrement sur les feuillets : $F_{0} =
\mathrm{Ob}\,\Delta_{n'}$ à la page 2, où la suite demande $\Delta_{n}$ ;
$\mathrm{Fl}\, C_{2}$ en tête de la troisième ligne de la page 3, où elle
demande $C_{1}$ ; et $\mathrm{Ob}\, C_{q}$ pour le dernier facteur de
$F_{n}$, où elle demande $C_{n+1}$. On rétablit la forme que la série impose.
La page 3 porte aussi des sous-ensembles $O_{n} \subset F_{n}$, un cylindre
$(F \times I) \sqcup_{\dot{F} \times I} \dot{F}$ et un produit de joints
$(O_{1}, F_{1}) * (O_{2}, F_{2}) * \cdots$, sans texte.}

\emph{Page 4 : quand un morphisme est-il une équivalence.} Le complexe
$\mathbb{Z}[F_{n+1}] \to \mathbb{Z}[F_{n}] \xrightarrow{t_{n} - s_{n}}
\mathbb{Z}[F_{n-1}]$ est un complexe, parce que
...
\emph{Page 5 : premier état de la construction.} Au dos d'une page de son
propre tapuscrit anglais sur les « Gr-stacks », le premier état des pages 6 à
9, où l'ensemble ordonné des classes s'appelle $I$ au lieu de $\mathbb{N}$ :
...
\]
que la page 10 réduira à $\mathrm{Card}\, I_{n} = 3^{n}$.\footnote{La page note
...
$\underline{B} = (\text{Gr-stacks})$ et nomme les $\widetilde{D}_{n}$ ; le terme
...
\item[b)] pour tout $\xi \in P_{n}$, $(P_{n})_{\leq \xi} \simeq P_{n'}$ pour un
$n' \leq n$ convenable ?
\end{enumerate}
...
exemples. Parmi les polytopes réguliers convexes, la condition b) --- toute face
...
Le disque hémisphérique n'est pas un polytope convexe, et c'est ce que la
question laisse ouvert. Cette remarque est la nôtre.}
...
valeurs dans $\mathbf{N}$, les entiers, après avoir biffé $\mathbb{N}$ ; il
est alors dans le cas qu'il a « en tête ». On garde $\mathbb{N}$, qui est le cas
