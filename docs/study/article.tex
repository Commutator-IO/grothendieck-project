...
\title{\Large A Machine Reads Grothendieck's Working Notes:\\ Transcribing and Encoding Mathematical Manuscripts}
...
\texttt{\textbackslash supplied\{…\}} & What the transcriber supplies and the page does not carry: a missing letter, a clipped word-end & \texttt{supplied resp="\#pass"}\\
\texttt{\textbackslash add\{…\}} & His own insertion: above the line, over a struck word, his ink on a typescript & \texttt{add}\\
...
The subset is policed twice, by the renderer that turns each file into the site's reading view and by the converter to TEI (Section 6), and both refusals now stop the build. An unknown environment is refused by both, naming the file; the converter also refuses a control sequence it would otherwise leave in the TEI as text, and a header line recording a pass that it cannot read. Until late September 2026 a refusal was a warning: the script exited cleanly, the deploy went on, and the refused batch was simply missing from the site. The count of leaked control sequences, which found some thirty files carrying them earlier that month, all corrected at source, is now that refusal rather than a check run on request. Of three failures recorded in the repository as passing the renderer silently, one is now caught: the converter no longer maps \texttt{\textbackslash underline}, so the macro in running prose is a control sequence left as text, and refused. The other two --- a backslash-newline inside inline mathematics, and a page-range declaration without a blank line before it --- still pass both scripts, and are found by reading the output.
...
<add><formula notation="TeX">\alpha_{i}</formula></add> sont des
isomorphismes (i.e. les <formula notation="TeX">g_{i}</formula> pleinement fidèles). [...]</p>
\end{lstlisting}

The last paragraph shows the distinction Section 9.2 takes up. The \(\alpha_i\) is his, written over the \(\varphi\) he struck, and is encoded as \texttt{<add>}; until 26 September 2026 the converter, following the one macro's declared sense, encoded it as \texttt{<supplied>} and credited it to the transcriber.
...
Until late September 2026 each TEI file was checked for well-formedness as it was written, where the checker was installed, and nothing more ran on the build; validation against \texttt{tei\_all} had last been run by hand on 5 September, over the 53 files that then existed. The edition now has a customisation of its own, on the model of its sibling on the Hopper ledgers, which learned the cost of going without one the hard way --- a corpus of well-formed files that were all invalid, because nothing had asked (Hua 2026). The ODD declares exactly the elements the converter emits: sixty-four, header included, which are the sixty-two it emitted before, \texttt{<add>} since his insertions were told apart from the transcriber's (Section 4.4), and the \texttt{<schemaRef>} by which each file names the ODD. It closes the value lists of the attributes that carry meaning: \texttt{@type} on \texttt{<div>}, \texttt{<figure>} and \texttt{<note>}, \texttt{@notation} on \texttt{<formula>} (\texttt{TeX}, \texttt{tikz-cd}), \texttt{@reason} on \texttt{<gap>}, and \texttt{@rend} wherever the converter writes it. It is a pure subset, so a file valid against it is valid TEI.

The RELAX NG schema is derived from the ODD on every build, with the Consortium's own stylesheets, and published beside it; every exported file names it in an \texttt{xml-model} processing instruction, so that an XML editor opening a downloaded transcription validates it unasked. The build then validates every file, with jing, against unmodified \texttt{tei\_all} and against the derived schema, every input pinned by version and digest; and it reads the ODD against the export in both directions, failing on an element used and not declared and on one declared and never used, since a subset that can only grow is the full schema again. The first run found all 363 files valid against \texttt{tei\_all}, and all of them invalid against the first draft of the ODD, in the same way: the two notes the header writes about the dating and the facsimile carry no \texttt{@type}. They belong to the edition and not to the apparatus, and the ODD now says that the attribute is absent there and only there, which the validator checks. A comparison of the TEI's reading view with the \texttt{.tex}'s, page by page, with every formula of both typeset by KaTeX, runs on the same build and found no difference. What is still not validated is the Schematron that \texttt{tei\_all} embeds, which is not run over the corpus, and the TeX inside each \texttt{<formula>}, which the schema sees as a string and only KaTeX reads.

The header's debt is paid in the same change. The export used to record the first pass and no other, so that the twenty-seven files carrying a later machine revision named one model and one date for a file two passes produced. Every pass the source's header records is now a \texttt{<change>} in \texttt{<revisionDesc>}, dated and pointing to the model that made it, with the source's own account of what changed; a model that revised a file without having first read it gets a responsibility statement of its own. Five files wrote a pass in a form no parser could read, one of them a first pass; they were rewritten at source, from the evidence of the file and its commits, and a record the converter cannot read is now refused rather than dropped.

\section{Interface and reproducibility}

\subsection{A facsimile the edition does not hold}

The site at \url{https://grothendieck.commutator.io} renders the transcription in one pane and Montpellier's facsimile in the other, and scrolling the transcription past a page break turns the facsimile to that page. The address of a reading is the page, not the batch: \texttt{\#140-1/2/p28} opens folder \texttt{140-1} at batch 2 with the transcription scrolled to page 28 and the facsimile turned to it, and every register below links in that form. The edition holds no page of the fonds: not in the repository, whose ignore file refuses PDF outright, and not on the server. The facsimile is Montpellier's own file.
...
Once the transcription is a constrained language rather than a document, a question put to the fonds becomes a script. The edition publishes seven such registers, each regenerated from the files and each resting, entry by entry, on a file and a line.

\emph{The hand} counts the apparatus per batch and per folder: of 505,415 words read by 26 September 2026, 39,397 were left illegible, 7.2 per cent; 26,428 readings were marked doubtful; he struck 20,633 times and inserted 5,585 times outside the transcriber's notes, the transcriber supplied 524 letters or words, and there are 3,230 marginal notes; there are 1,330 commutative diagrams and 156 drawings described in notes. Figure~\ref{fig:hand} places each folder's share of unread words against its inventory dating. The figure is a count of the apparatus and not of the ink, and says so in its caption: an \texttt{\textbackslash ill\{\}} is a word one pass could not read, a fact about the hand and the pass at once.

\input{fig-hand.tex}

\emph{The timeline} draws each folder's inventory dating as a range --- dark where the archivists read it, light where they deduced it, fading where it is open-ended --- and the dates on the leaves as dots over it (Figure~\ref{fig:dating} is its summary); the parse of a dating keeps each end's qualifier, \q{vers}, \q{à partir de}, \q{avant}, because collapsing \q{1965-[vers 1971]} to 1965--1971 would throw away the distinction the page exists to show. \emph{The letters} is a register, not a text: 42 letters in 24 folders, 1959 to 1984, each with its date as written, its direction, its form, and how the transcription gives it, private correspondents unnamed. \emph{The index} leads from a person, a work of his cited by volume (EGA, SGA, FGA), or a subject, to the folder and batch where it occurs. \emph{The maps} are co-occurrence networks in the manner of bibliometric mapping --- who is named together in a folder, which subjects share folders, which folders cite which volumes --- whose every edge is evidence and whose layout is computed once, with a fixed seed, so that the figure is the same on every load. A slider over the years shows each map as it stood when the folders dated by then are counted --- by the midpoint of the inventory's dating, which the control says is a tendency and not a date.

The last two registers are the mathematics itself, gathered out of the pages it stands on. \emph{The diagrams} are the 1,330 \texttt{tikzcd} environments of the transcriptions, in 110 folders, shown one to a slide by the reading view's own renderer; \emph{the formulas} are the 11,633 displayed formulas, in 124 folders, set by KaTeX with their apparatus inside. Each opens on its richest, ranked by a count that says what it measures: the distinct symbols a diagram or a formula uses --- a letter in a given alphabet, so that \(\mathcal{C}\) is not \(C\), a named symbol, an operator --- with, for a diagram, its nodes and arrows, and, for a formula, its size. The ranking finds what carries the most, not what matters most, which no count can find; and the richest diagram of the fonds, nineteen objects and twenty-nine arrows on page 28 of folder \texttt{140-1}, a draft of the \emph{Longue Marche}, is a fair sample of what the transcriptions ask a renderer to draw. Gathering them exposed faults a page of prose had hidden: every diagram of a page took its arrowheads from the first diagram's definitions, so that a hidden first diagram left every arrow headless; a label was set a fixed distance from its arrow whatever its size, and a long one sat on the line; and KaTeX's stretchy braces, painted at a negative depth, vanished under any background. None was a fault of the transcriptions, and all were found by looking at the mathematics one piece at a time rather than a page at a time.
...
\emph{One macro carried two senses.} \texttt{\textbackslash add} was declared as the transcriber's addition --- a missing letter, an implied word --- and mapped to \texttt{<supplied resp="\#pass">}. Read in context, most of its 6,144 uses were his: a word written above the line with a caret, a substitution over a struck word, the \(\alpha_i\) of Section 4.5. Sixteen batch headers stated as much for their typescripts, where his ink insertions were marked with it by convention. The TEI credited to the model several thousand of his insertions, and a count of them from the TEI was wrong by an order of magnitude. The same failure was found in the Hopper edition, on a smaller scale, and resolved there by dividing the macro in two; it has been resolved here the same way. \texttt{\textbackslash supplied} now carries the transcriber's sense and maps to \texttt{<supplied resp="\#pass">}; \texttt{\textbackslash add} carries his and maps to \texttt{<add>}. The corpus was divided between them by a rule checkable in the file rather than by a fresh reading: an addition glued to the word it completes, a letter or digit immediately before or after it, is the transcriber's, and one standing free is his. That assigns 529 uses to \texttt{\textbackslash supplied} and leaves 5,615 as \texttt{\textbackslash add}. The residue is the rule's guess and not a reading. A free-standing supply --- his sign \q{=} for \q{même} expanded in full, a closing parenthesis restored at a clipped edge --- still reads as his, and a prefix he inserted before a word, now \q{\texttt{\textbackslash supplied\{quasi-\}torseur}}, has been handed to the transcriber. Only a person reading the facsimiles will settle those. Given one macro for an insertion, the model used it for both kinds, and the lesson generalises: a macro whose declared sense is narrower than its natural sense will be stretched to the natural one.
...
The method permits a reading of a sixteen-thousand-page mathematical archive to be produced in months by one person's attention and a model's time, cited by folder, batch, page and pass, exported to TEI with its mathematics intact, placed beside an institution's own facsimile without copying it, and mined for registers of names, dates, letters and hands that each rest on a line. It does not produce a checked transcription; it does not encode the mathematics in a form a TEI processor can reason over, though it validates what it does encode against a customisation of its own; it does not decide who is a public figure with authority; and it does not remove the need for the person it was designed around.
...
The encoding work this article first listed as owed is done: an ODD declaring exactly what the converter emits and closing its value lists, a schema derived from it that every file names, validation of every file against it and against \texttt{tei\_all} on every build, a header that records every pass, and a refused file that stops the build, so that a deposit is TEI and not XML. What remains is stated as two pieces of work. The first is the reading against the facsimile of what the rule of Section 9.2 divided mechanically: which of the 5,615 free-standing insertions are his and which of the 529 glued ones are the transcriber's. The second is the human check, which is the one on which every claim of the edition depends, and which is a question for the mathematicians who can read this hand, not for a model.
