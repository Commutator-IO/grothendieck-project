 ...
 */
const FIGURE_RIGHTS = 'Figure by the author, drawn from the edition’s derived data or, for the schema, by hand; CC BY 4.0. No image of the fonds is reproduced.';
...
const CITATIONS = [
  ...
  { id: 'montpellier', forms: ['Université de Montpellier n.d.'] },
  { id: 'w3c2014', forms: ['W3C 2014'] },
];
const cited = new Set();
...
function inline(tex, { cite = true } = {}) {
  ...
  // Inline mathematics, \( … \). The journal's schema admits no <formula>
  // — the element the edition carries Grothendieck's mathematics in — so the
  // article's few symbols are written in Unicode, italic, a subscript as a
  // subscript. The table is closed: a symbol it does not know fails, rather
  // than reaching the file as TeX nobody will typeset.
  const SYM = { '\\varphi': 'φ', '\\alpha': 'α', '\\omega': 'ω', '\\Sigma': 'Σ', '\\mathcal{C}': '𝒞' };
  s = s.replace(/\\\(([\s\S]*?)\\\)/g, (_, m) => {
    let t = m.trim();
    for (const [k, v] of Object.entries(SYM)) t = t.split(k).join(v);
    t = t.replace(/_\{?([a-z0-9])\}?/g, '<hi rend="subscript">$1</hi>');
    if (/\\/.test(t)) throw new Error(`article-tei: no Unicode for \\(${m}\\)`);
    return `<hi rend="italic">${t}</hi>`;
  });
  ...
  s = s.replace(/\\lf\{([^}]*)\}\{([^}]*)\}\{([^}]*)\}/g, (_, folder, batch, label) =>
    `<ref target="https://grothendieck.commutator.io/#${folder}/${batch}">${inline(label)}</ref>`);
...
<sourceDesc>
<p>No source, born digital. Derived from the LaTeX source <code>docs/study/article.tex</code> of the repository <ref target="https://github.com/Commutator-IO/grothendieck-project">Commutator-IO/grothendieck-project</ref> by <code>docs/study/tei.mjs</code>, and validated against the journal’s schema on every build.</p>
</sourceDesc>
</fileDesc>
<encodingDesc>
<projectDesc>
...
process.stdout.write(
  `article-tei: ${mainDivs.length} divisions, ${figureCount} figures, ${tableCount} tables, ` +
    ...
    `      -> docs/study/submission/article.xml with ${figureFiles.length} PNG(s) at 300 dpi; served at /article/grothendieck-jtei.xml\n`,
);
